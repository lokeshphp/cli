...
long long LongLongBigM = 99999999999;
...
void SmartQ::PrintStats()
{
#ifdef ALLSTATS
   printf("c Empty Buckets: %18lld       \n", statEmpty);
   printf("c Expanded Nodes: %17ld       Expanded Buckets: %13ld\n",
	  statExpandedNodes, statExpandedBuckets);
   printf("c Bucket inserts:  %16ld       Position evals: %15ld\n",
	  statInsert, statPosEval);
#endif
}
...
#endif
{
   ...
   Arc2 *arc, *lastArc;            // last arc of the current node
   Bucket *bckOld, *bckNew;
   long long maxVarde = 0;
   int nAvsokning = 0;
...
//#endif
	 ...
	 if( currentNode->dist + arc->len < newNode->dist )
		 fprintf(FilPek, "+++till nod %d nodcost %d arclen %d (tot %d), caliber %d\n",
			  ...
			  (int)newNode->dist, (int)arc->len, (int)(currentNode->dist + arc->len),
			  (int)CALIBER(newNode));
	 else
	     fprintf(FilPek, "---till nod %d nodcost %d arclen %d (tot %d), caliber %d\n",
			  model->Dijkstra->sp->nodeId(newNode)+model->Dijkstra->node_min-1,
...
//   int pos;
	 if(reached == true){
//		 FilPek = fopen("CheckDijkstra.txt", "a+");
//		 fprintf(FilPek, "Uppnystning, total kostnad %I64d\n", sink->dist - source->dist);
		 *OptCost = sink->dist - source->dist;
//printf("Max kostnad %d, sink kost %d\n", maxVarde, sink->dist-source->dist);

//		 pos = 0;
//		 for(newNode = sink;newNode != source; newNode = newNode->parent){
//			 fprintf(FilPek,"P%d:n%d:c%I64d:Nasta%I64d ", 
//				 pos, sp->nodeId(newNode)-1, newNode->dist, newNode->dist-newNode->parent->dist);
//			 pos++;
//		 }
//		 fprintf(FilPek, "n%d:c%I64d\n", sp->nodeId(newNode)-1, newNode->dist);
//		 fclose(FilPek);

	 }else{
		 printf("+");
//		 printf("Misslyckades med uppnystningen, kunde ej komma till slutnoden fran startnoden...\n");
		 *OptCost = LongLongBigM;

//		 for (currentNode = model->Dijkstra->nodes; 
//			currentNode < model->Dijkstra->nodes + model->Dijkstra->nNoder; 
//			currentNode++){
//				if(currentNode->dist < VERY_FAR / 2 && currentNode->dist > 0){
//					PFerrlog(0, "Nod %d (verkl %d) dist %I64d\n",
//					model->Dijkstra->sp->nodeId(currentNode)+model->Dijkstra->node_min-1,
//					model->Noder[model->Dijkstra->sp->nodeId(currentNode)+model->Dijkstra->node_min-1]->NodID, 
//					currentNode->dist);
//				}
...
//		printf("Avsokta noder %d, kostnad %d\n",nAvsokning, *OptCost);
...
//#else
   ...
   for (currentNode = model.Dijkstra.nodes; 
	  currentNode < model.Dijkstra.nodes + model.Dijkstra.nNoder; 
	  currentNode++){
		if(currentNode->where == IN_F){
//		if(currentNode->dist < VERY_FAR / 2){
			errlog("Nod %d dist %I64d\n",
			  model.Dijkstra.sp->nodeId(currentNode)+model.Dijkstra.node_min-1,
			  currentNode->dist);
		}
	}
