...
int calc_boundingBoxAutoRoute() {
	...
	model.boundingBox.xMin = minX - model.paramsAutoRoute.factorExtraCover * distGrader;
	model.boundingBox.yMin = minY - model.paramsAutoRoute.factorExtraCover * distGrader;
	if (model.boundingBox.yMin < -90)
...
int evalCostArc(strAutoCells* cell, int pos1, double y1, double x1, double y2, double x2) {
	double cost, dist, kvotBad2;
...
strAutoCells* addSmallerCells(strAutoCells* thisCell, double xBas, double yBas, int levelTo) {
	...
	for (i = 0; i < thisCell->nYsmall; i++) {
		for (i1 = 0; i1 < thisCell->nXsmall; i1++) {
			...
			cells[pos].x = x;
			cells[pos].y = y;
			evalCostArc(&(cells[pos]), 0, y, x, y, x + model.paramsAutoRoute.discretizationSizeLevel[levelTo]); // horizontal
			evalCostArc(&(cells[pos]), 1, y, x, y + model.paramsAutoRoute.discretizationSizeLevel[levelTo], x + model.paramsAutoRoute.discretizationSizeLevel[levelTo]); // diagonal
			evalCostArc(&(cells[pos]), 2, y, x, y + model.paramsAutoRoute.discretizationSizeLevel[levelTo], x); // vertical
			x += model.paramsAutoRoute.discretizationSizeLevel[levelTo];
			evalCostArc(&(cells[pos]), 3, y, x, y + model.paramsAutoRoute.discretizationSizeLevel[levelTo], x - model.paramsAutoRoute.discretizationSizeLevel[levelTo]); // diagonal
...
int writeAllAutoNodesToGeojson()
{
	...
	char* namn;
	namn = (char*)malloc2(256 * sizeof(char));
	sprintf(namn, "%s/autoNodes.geojson", model.params.indataPath.c_str());
	...
	for (i = 0; i < model.paramsAutoRoute.nYbasLevel; i++) {
		for (i1 = 0; i1 < model.paramsAutoRoute.nXbasLevel; i1++) {
			...
			if (model.autoRoute[pos].smallerCells != NULL) {
...
int writeAllAutoArcsToGeojson()
{
	...
	char* namn;
	namn = (char*)malloc2(256 * sizeof(char));
	sprintf(namn, "%s/autoArcs.geojson", model.params.indataPath.c_str());
	filpekG = fopen(namn, "w");
...
int createCells() {
	...
	for (i = 0; i < model.paramsAutoRoute.nYbasLevel; i++) {
		for (i1 = 0; i1 < model.paramsAutoRoute.nXbasLevel; i1++) {
			...
			evalCostArc(&(model.autoRoute[pos]), 0, y, x, y, x + model.paramsAutoRoute.discretizationSizeLevel[0]); // horizontal
			evalCostArc(&(model.autoRoute[pos]), 1, y, x, y + model.paramsAutoRoute.discretizationSizeLevel[0], x + model.paramsAutoRoute.discretizationSizeLevel[0]); // diagonal
			evalCostArc(&(model.autoRoute[pos]), 2, y, x, y + model.paramsAutoRoute.discretizationSizeLevel[0], x); // vertical
			evalCostArc(&(model.autoRoute[pos]), 3, y, x + model.paramsAutoRoute.discretizationSizeLevel[0], y + model.paramsAutoRoute.discretizationSizeLevel[0], x); // diagonal
...
double addAutoArcSmall(int cellPos1, int cellSmall1, int posTmp1, int cellPos2, int cellSmall2, int posTmp2, int posUse) {
	...
	if (cellPos1 >= model.paramsAutoRoute.nCellsBase || cellPos2 >= model.paramsAutoRoute.nCellsBase) {
		...
		dist = model.autoRoute[cellUse].arcDistance[posUse];
		cost = 1.1 * model.autoRoute[cellUse].arcCost[posUse];
	}
	else {
		if (cellPos1 < cellPos2) {
			cellUse = cellPos1;
		...
		cost = 1.1 * model.autoRoute[cellUse].smallerCells[cellSmallUse].arcCost[posUse];
	}
	model.arc[model.nArcs].fromLevel = cellPos1;
	model.arc[model.nArcs].fromPointNr = posTmp1;
...
int addArcsFromStartNode(int yPos, int xPos, int level) {
	...
	evalCostArc(&(model.autoRoute[posEnd]), posTmp, model.paramsAutoRoute.startPoint_lat, model.paramsAutoRoute.startPoint_lon,
		y, x);
	model.Noder[nodNr].outArcNr[nodPos] = model.nArcs;
...
int addArcsSmallFromStartNode(int yPos, int xPos, int level) {
	int nodPos, posEnd, posTmp, cellPos, nodNr, yPos2, xPos2, cellPos2;
	double y, x, cost;
...
int addArcsSmallToEndNode(int yPos, int xPos, int level) {
	int nodPos, posEnd, posTmp, cellPos, nodNr, nodEnd, cellPos2, yPos2, xPos2;
	double y, x, cost;
...
int createCellNetwork() {
	...
	for (i = 0; i < model.paramsAutoRoute.nYbasLevel; i++) {
		for (i1 = 0; i1 < model.paramsAutoRoute.nXbasLevel; i1++) {
			...
			if (i > 0) {
				...
				if(i1 > 0)
					model.autoRoute[pos].nodeNr[0] = model.autoRoute[pos-1].nodeNr[1];
				else {
					addAutoNode(pos, 0, -1);
				}
				addAutoNode(pos, 1, -1);
			...
			if(i1 > 0)
				model.autoRoute[pos].nodeNr[2] = model.autoRoute[pos - 1].nodeNr[3];
			else {
				addAutoNode(pos, 2, -1);
			}
			addAutoNode(pos, 3, -1);
	...
	int saveNodes = 0;
	if (saveNodes == 1)
		writeAllAutoNodesToGeojson();

	int saveArcs = 0;
	if(saveArcs == 1)
		writeAllAutoArcsToGeojson();
	checkMinnesAnvandning(__LINE__);
...
int writeSolutionToJson_autoRoute(std::string filename)
{
	FILE* filpekG;
	...
	if (SKRIV_UT_NOTHING == 0) {
		sprintf(namn, "%s/autoPath.txt", model.params.indataPath.c_str());
	...
	fprintf(filpekG, "{ \"type\": \"Feature\",\n\"geometry\": { \"type\": \"MultiLineString\",\n\"coordinates\": [ [\n");
	int iPos, arcNr;
	double totCost = 0, totDist = 0, y, x;
...
int genAutoRoute(std::string inputPath, std::string resultName){
	...
	if (Reached == true) {
		...
		if (model.nBVArcs < 2) {
			...
			writeSolutionToJson_autoRoute(resultName);//  model.params.resultPath + "/resAutoRoute.json");
		checkMinnesAnvandning(__LINE__);
	}
	else {
