...
int createCells() {
	...
	for (i = 0; i < model.paramsAutoRoute.nYbasLevel; i++) {
		for (i1 = 0; i1 < model.paramsAutoRoute.nXbasLevel; i1++) {
			...
			evalCostArc(&(model.autoRoute[pos]), 1, y, x, y + model.paramsAutoRoute.discretizationSizeLevel[0], x + model.paramsAutoRoute.discretizationSizeLevel[0]); // diagonal up
			evalCostArc(&(model.autoRoute[pos]), 2, y, x, y + model.paramsAutoRoute.discretizationSizeLevel[0], x); // vertical
			evalCostArc(&(model.autoRoute[pos]), 3, y, x + model.paramsAutoRoute.discretizationSizeLevel[0], y + model.paramsAutoRoute.discretizationSizeLevel[0], x); // diagonal down
...
double addAutoArcSmallTss(int tssNr, int posItss, int prev_posItss, int cellPos, int cellSmall, int posTmp, double dist, int direction) {
	...
	if (direction == -1) { // in to tss
		if (prev_posItss >= 0) {
			...
			model.arc[model.nArcs].fromPointNr = cellSmall;
			model.arc[model.nArcs].fromTime = posTmp;
		}
...
double addAutoArcExtra(int arcNr1, int arcNr2, int nod1, int nod2, double dist) {
	double cost;
	int nodPos;
	if (model.nArcs >= model.nAllocArcs) {
		model.nAllocArcs += 1000000;
	...
	model.arc[model.nArcs].toLevel = model.arc[arcNr1].toLevel;
	model.arc[model.nArcs].toPointNr = model.arc[arcNr1].toPointNr;
	model.arc[model.nArcs].toTime = model.arc[arcNr1].toTime;
	model.arc[model.nArcs].distance = dist;
	model.arc[model.nArcs].totCost = dist;
	...
	*cellSmallUse = xPosUse2 + model.paramsAutoRoute.nDiscreteSizeLevel[1] * yPosUse2;
	if (*cellNrUse > cellNr || (*cellNrUse == cellNr && *cellSmallUse >= cellSmall1)) {
		...
		if (direction >= 4) {
			...
			if (direction >= 8)
				direction -= 4;
			evalCostArc(&(model.autoRoute[cellNr].smallerCells[cellSmall1]), direction, y1, x1, y2, x2);
		}
...
int getNodFromArcNr(int arcNr, int fromTo) {
...
int addArcsAroundSolution()
{
	...
	int iPos, arcNr, nEjBattre, i1, arcNr1, nod1, nod2;
	double totCost = 0, totDist = 0, y, x, y1, x1, costNod1, costDiff, kvotBad, dist, cost;
	for (iPos = 1; iPos < model.nBVArcs; iPos++)
	{
		arcNr = model.BVArc[iPos];
		nod1 = getNodFromArcNr(arcNr, 0);
		...
		for (i1 = iPos + 1; i1 < model.nBVArcs; i1++) {
			...
			kvotBad = check_map_badKvot_auto(y, x, y1, x1, 0, -1);
			if (kvotBad < 0.001) { // only add allowed arcs
				dist = estimateLargeCircleDistance_km(y, x, y1, x1);
				nod2 = getNodFromArcNr(arcNr1, 1);
				costDiff = (model.Dijkstra.nodes - model.Dijkstra.node_min + nod2)->dist / model.Dijkstra.FAKTOR_NATVERK - costNod1;
				if (costDiff > dist * 0.99) {
					// add arc
					cost = addAutoArcExtra(arcNr, arcNr1, nod1, nod2, dist);
				}
