...
struct strWeather
{
	double timeIntervall_expected; // number of hours between data points

	char* weatherFileTypeName;
	int defaultValueMore; // 1 if the default value 9999 is higher than allowed values, 0 otherwise
	//int nElement;
	int nTimeIntervals;
...
public:
	...
	*/
	...
	int Get_BlockSize(int z, int* pnXSize, int* pnYSize) {
  ...
  // read the scanline into the dynamically allocated row-buffer       
	...
	GByte* GetRasterBand_intArrTest(int z, strPhysRaster* rasterData, strBoundBox boundingBox) {
		...
		int nNotValid = 0, posTmp, posTmp2, nXnotValid, nXnotValid0;
		double xPosFrac1, yPosFrac1, xPosFrac2, yPosFrac2;
		//unsigned short* valueCell;
		GDALRasterBand* poBand = rasterDataset->GetRasterBand(z);
		...
		if (pnXSize == NCOLS) {
			...
			errlog("boundingBox changing temporarily in _intArrTest to %.3lf %.3lf\n", bb_xMin, bb_xMax);
		}
		else {
		...
		if (bb_xMin < min_lon) {
			nXnotValid0 = n_xBlocks * pnXSize - NCOLS;
			xPosFrac1 -= nXnotValid0 / (double)pnXSize;
		}
		else
			nXnotValid0 = 0;
		...
		if (pnXSize == NCOLS) {
			...
			//xMin = (int)xPosFrac1; // fix_minMaxFromFrac(xPosFrac1, 0, nXBlocks - 1);
			//xMax = (int)xPosFrac2; // fix_minMaxFromFrac(xPosFrac2, 0, nXBlocks - 1);
			xMin = roundDown(xPosFrac1); // fix_minMaxFromFrac(xPosFrac1, 0, nXBlocks - 1);
			xMax = roundDown(xPosFrac2); // fix_minMaxFromFrac(xPosFrac2, 0, nXBlocks - 1);
		}
		yMin = fix_minMaxFromFrac(yPosFrac1, 0, nYBlocks - 1);
		yMax = fix_minMaxFromFrac(yPosFrac2, 0, nYBlocks - 1);
		//printf("bounding box %.3lf %.3lf %.3lf %.3lf\n",
		...
		//printf("blocks to open for physical map x %d %d y %d %d\n", xMin, xMax, yMin, yMax);
		rasterData->minLongitude = min_lon + (double)(xMin * pnXSize + nXnotValid0) * size_col; // pnXSize / NCOLS * (max_lon - min_lon);
		rasterData->minLatitude = max_lat - (double)(yMax + 1) * pnYSize * size_row; // pnYSize / NROWS * (max_lat - min_lat);
		...
		//printf("alloc pabyData size %d x %d = %d\n", pnXSize, pnYSize, pnXSize * pnYSize);
		rasterData->nCols = (xMax - xMin + 1) * pnXSize - nXnotValid - nXnotValid0;
		rasterData->nRows = (yMax - yMin + 1) * pnYSize;
		rasterData->size_col = size_col;
		rasterData->size_row = size_row;
		rasterData->nBlock_x = nXBlocks; // pnXSize;
	...
	int GetRasterValues_intArray(int z, int nElement, double* x, double* y, int* returnArr) {

		int pnXSize, pnYSize, xUse;
		int xPos, yPos, iYBlock;
		double xPosFrac1, yPosFrac1;
		GDALRasterBand* poBand = rasterDataset->GetRasterBand(z);
		...
		int n_xBlocks, n_yBlocks, nXValid, nYValid;

		n_xBlocks = (int)((double)NCOLS / pnXSize);
		if (n_xBlocks * pnXSize < NCOLS)
			n_xBlocks++;
		n_yBlocks = (int)((double)NCOLS / pnXSize);
		if (n_yBlocks * pnXSize < NCOLS)
			n_yBlocks++;

		GByte* pabyData = (GByte*)CPLMalloc(pnXSize * pnYSize);

		for (int i0 = 0; i0 < nElement; i0++) {
			if (x[i0] < min_lon)
				x[i0] += 360.0;
			if (x[i0] > max_lon)
				x[i0] -= 360.0;
			xPosFrac1 = (x[i0] - min_lon) * NCOLS / pnXSize / (max_lon - min_lon);
			yPosFrac1 = (max_lat - y[i0]) * NROWS / pnYSize / (max_lat - min_lat);

			xUse = roundDown(xPosFrac1); // fix_minMaxFromFrac(xPosFrac1, 0, nXBlocks - 1);
			iYBlock = roundDown(yPosFrac1);
			if (xUse < 0 || xUse >= n_xBlocks || iYBlock < 0 || iYBlock >= n_yBlocks) {
				returnArr[i0] = 0;
			}
			else {
				poBand->ReadBlock(xUse, iYBlock, pabyData);
				...
				xPos = (xPosFrac1 - xUse) * pnXSize;
				yPos = (yPosFrac1 - iYBlock) * pnYSize;
				if(xPos < nXValid && yPos < nYValid)
					returnArr[i0] = pabyData[xPos + yPos * pnXSize];
				else
					returnArr[i0] = 0;
			}
		}
	...
	long long GetSecondsFromUTC_metadataBand(int z) {
		long long nSecondsUTC = -1;
		GDALRasterBand* poBand = rasterDataset->GetRasterBand(z);
		nSecondsUTC = getSecondsFromUTC(poBand->GetMetadataItem("GRIB_VALID_TIME"));
	...
	void GetRasterValues_realAllBands(strWeather* weatherData, int zPosBas, double filKvot, int startBand = 0) {
		...
		GDALDataType bandType;
		FILE* filpek = NULL;
		int nAlloc3 = weatherData->nCols * weatherData->nRows, pos, pos2;
					...
					if (xPosNu + pnXSize > weatherData->nCols) {
						...
						if (max_lon - min_lon > 280) {
							if (useMinX > weatherData->minX) {
								x0b = NCOLS - xPosNu;
								x2 = NCOLS;
								if (xPosNu < weatherData->nCols)
									x2 = NCOLS;
								else
									x2 = x0b + weatherData->nCols;
								xPosNu2 = -x0b;
							}
						}
					}
	...
	void GetRasterValues_realAllBandsUp(strWeather* weatherData, int zPosBas, double filKvot, int startBand = 0) {
	...
	void GetRasterValues_realAllBands_fixBandNr(strWeather* weatherData, int zPosBas, int nMaxBands, double filKvot) {
		...
		nBands = rasterDataset->GetRasterCount();
		//printf("nBands %d\n", nBands);
		for (z = 1; z <= nBands && z <= nMaxBands - zPosBas; z++) {
			zNu = zPosBas + z - 1;
			if (zNu == 297)
				zNu = zNu;
			poBand = rasterDataset->GetRasterBand(z);
				...
				for (iXBlock = xMin; iXBlock <= xMax; iXBlock++)
				{
					...
					for (iY = y0; iY < y1; iY++) {
						for (iX = x0; iX < x1; iX++) {
							...
							pos = iX + xPosNu + weatherData->nCols * (iY + yPosNu);
							if (pos < 0 || pos >= nAlloc3 || pos2 < 0 || pos2 >= pnXSize * pnYSize) {
								printf("ERROR! pos %d nAlloc3 %d pos2 %d pnXSize %d, pnYSize %d xx %d\n",
						...
						for (iX = x0b; iX < x2; iX++) {
							...
							if (pabyData[pos2] < 9998) {
								pos = iX + xPosNu2 + weatherData->nCols * (iY + yPosNu);
								if (pos < 0 || pos >= nAlloc3 || pos2 < 0 || pos2 >= pnXSize * pnYSize) {
									printf("ERROR2! pos %d nAlloc3 %d pos2 %d pnXSize %d, pnYSize %d xx %d\n",
										pos, nAlloc3, pos2, pnXSize, pnYSize, pnXSize * pnYSize);
									errlog("ERROR! pos %d nAlloc3 %d pos2 %d pnXSize %d, pnYSize %d xx %d\n",
										pos, nAlloc3, pos2, pnXSize, pnYSize, pnXSize * pnYSize);
								}
	...
	void GetRasterValues_realHindCastBands(strWeather* weatherData, int bandPosStart, int bandPosEnd, int posStartSave, double filKvot) {
		...
		nBands = rasterDataset->GetRasterCount();
		//printf("nBands %d\n", nBands);
		for (z = bandPosStart + 1; z <= bandPosEnd + 1; z++) {
			zNu = posStartSave + z - bandPosStart - 1;
			if (zNu == 297)
				zNu = zNu;
			poBand = rasterDataset->GetRasterBand(z);
			if (weatherData->secondsUTC != NULL)
	...
	void GetRasterValues_realAllBandsTest2(strWeather* weatherData, int zPosBas) {
		...
		GDALDataType bandType;
		FILE* filpek = NULL;
		int nAlloc3 = weatherData->nCols * weatherData->nRows;
	...
	float** GetRasterValues_realAllBandsTest(strWeather* weatherData, int zPosBas) {
		...
		for (z = 1; z <= nBands; z++) {
			...
			int nAlloc2 = 2*weatherData->nCols * weatherData->nRows;
			valueCell[zNu] = (float*)malloc(nAlloc2 * sizeof(float));
				...
				for (iXBlock = xMin; iXBlock <= xMax; iXBlock++)
				{
					...
					for (iY = y0; iY < y1; iY++) {
						for (iX = x0; iX < x1; iX++) {
							pos2 = iX + iY * pnXSize;
							pos = iX + xPosNu + weatherData->nCols * (iY + yPosNu);
							if (pos < 0 || pos >= nAlloc2 || pos2 < 0 || pos2 >= pnXSize * pnYSize)
								printf("ERROR!\n");
							if (iY < nYValid && iX < nXValid) {
								if (pabyData[pos2] < 9998)
									valueCell[zNu][pos] = pabyData[pos2];
							}
							else
								valueCell[zNu][pos] = 0;
						}
