...
int USE_KVOTKOST = 1;
int USE_KVOTCOST_CORRIDORS = 0; // 1 if a factor should be multiplied to a corridor cost (old way), 0 if a minCostValue should be deducted;
double DEFAULT_KVOTMINCOST = 0.8;
...
double calcArcTimeCost(int tidInt, int speedSettingNr, int fromLevel, int toLevel, int restrictedAreaNr, double* calmWaterSpeed, double fuelFactorMain) {
	...
	double tidWait;
	if (fromLevel >= 0)
		tidWait = model.network.physicalLev[fromLevel].tidWait;
	else
		tidWait = 0;
	double tidStart = tidInt * model.params.tIndexGerH;
	double tidTot = tidStart + tidWait;
	double distNu, fuelTot_main = 0, fuelTot_aux = 0, safetyTot = 0, tmp1;
	...
	int i, ii, badSpeed = 0, tidIntForecast, legNr, firstTime = 1;
	...
	//	printGlobal = 0;
	if (*calmWaterSpeed < 0)
		*calmWaterSpeed = eval_calmWaterSpeed(speedSettingNr, fromLevel, toLevel, restrictedAreaNr);
	...
	for (i = 0; i < model.weatherFunctions.nCheckPoints; i++) {
		...
		//if (fuelFactorMain < 0)
		fuelConsumption_main = eval_fuelConsumption_both(speedSettingNr, &fuelConsumption_aux, fromLevel, toLevel, restrictedAreaNr, *calmWaterSpeed);
		//else
		...
		if (fromLevel < 0 && toLevel < 0) {
			if (firstTime == 1) {
				...
				//fuelUsage_aux -= fuelConsumption_aux * model.network.channel[-fromLevel - 1].waitingTime; // removed 20250501, not included in timeArc
		...
		if (fromLevel >= 0 && i == 0) {
			fuelUsage_aux += fuelConsumption_aux * tidWait;
		}
		...
		legNr = getLegNrFromLevels(fromLevel, toLevel);
		if (model.params.includeNazanin_safety >= 1)
			eval_safety_nazanin(legNr, speedOverGround, (*calmWaterSpeed) - speedDiffWindWave, windSpeed, absWindDirDiff, waveHeight, wavePeriod, rel_waveDir, iceCover);
		else
			eval_safety(legNr, iceCover);
...
double calcDelayedArcTimeCost(int fromLevel, int toLevel, int restrictedAreaNr, int speedSettingNr, double calmWaterSpeed, double fuelFactorMain, double factorDelay, double dist, double speedDiff) {
	...
	double timeArc, fuelConsumption_main, fuelConsumption_aux, fuelUsage_main, fuelUsage_aux, tidWait;
	...
	int legNr;
	if (fromLevel >= 0)
		legNr = model.network.physicalLev[fromLevel].legNr;
	else
		legNr = model.network.channel[-fromLevel - 1].legNr;
	...
	if (fuelFactorMain < 0)
		...
		fuelConsumption_main = eval_fuelConsumption_both(model.functions.speedSetting95MCR_use[legNr], &fuelConsumption_aux, -1, -100, restrictedAreaNr, calmWaterSpeed) * fuelFactorMain;
	fuelUsage_main = fuelConsumption_main * timeArc;
	fuelUsage_aux = fuelConsumption_aux * timeArc;
	...
	if (fromLevel >= 0) {
		timeArc += model.network.physicalLev[fromLevel].tidWait;
		fuelUsage_aux += fuelConsumption_aux * model.network.physicalLev[fromLevel].tidWait;
	}
...
double calcArcTimeCostChannel(int t, int speedSettingNr, int channelNr, int restrictedAreaNr, double* calmWaterSpeed) {
	...
	if (fixTime > 0) {
		...
		*calmWaterSpeed = eval_calmWaterSpeed(speedSettingNr, -channelNr - 1, -channelNr - 1, restrictedAreaNr);
	...
	for (i = 0; i < model.weatherFunctions.nCheckPoints; i++) {
		...
		if (fixTime <= 0) {
			...
			fuelConsumption_main = eval_fuelConsumption_both(speedSettingNr, &fuelConsumption_aux, -channelNr - 1, -channelNr - 1, restrictedAreaNr, *calmWaterSpeed);
			fuelUsage_main = fuelConsumption_main * timeArc;
			fuelUsage_aux = fuelConsumption_aux * timeArc;
			...
			legNr = getLegNrFromLevels(-channelNr - 1, -channelNr - 1);
			if (model.params.includeNazanin_safety >= 1)
				eval_safety_nazanin(legNr, speedOverGround, (*calmWaterSpeed) - speedDiffWindWave, windSpeed, absWindDirDiff, waveHeight, wavePeriod, rel_waveDir, iceCover);
			else
				eval_safety(legNr, iceCover);
			...
			fuelConsumption_main = eval_fuelConsumption_both(0, &fuelConsumption_aux, -channelNr - 1, -channelNr - 1, restrictedAreaNr, *calmWaterSpeed);
			fuelUsage_main = fuelConsumption_main * timeArc;
			fuelUsage_aux = fuelConsumption_aux * timeArc;
...
double calcDelayedArcTimeCostChannel(int t, int restrictedAreaNr, int speedSettingNr, int channelNr, double delayFactor, double* calmWaterSpeed) {
	...
	fuelConsumption_main = eval_fuelConsumption_both(speedSettingNr, &fuelConsumption_aux, channelNr, channelNr, restrictedAreaNr, *calmWaterSpeed);
	//}
	//else {
...
int addEndBage(int thisLevel, int pos1, int nextLevel, int i3, int nodNr2, int i2b) {
	...
	model.arc[arcNr].outNodePos = i2b;
...
int getLegNrFromLevels(int thisLevel, int nextLevel) {
	if (thisLevel >= 0)
		return model.network.physicalLev[thisLevel].legNr;
	else
		return model.network.channel[-thisLevel - 1].legNr;
}


int addEnBage_AB(int thisLevel, int pos1, int nextLevel, int pos2, int i2b, int restrictedAreaNr, int tPos, int i4, int* setupCheckPoints, int min_t, int max_t,
	double fuelQualityKvot, double extraAreaCostKvot, int runAlt)
{
	...
	int timeInt, nSpeedSettings, posDiff, legNr;
	...
	//if (thisLevel == 6)
	//	printf("thisLev %d\n", thisLevel);
	nSpeedSettings = determine_nSpeedSettingsToUse(thisLevel, nextLevel, restrictedAreaNr);
	if (thisLevel >= 0) {
	...
	if (thisLevel >= 0 || nextLevel >= 0) {
		...
		if (thisLevel == 79)
			thisLevel = thisLevel;
		if (model.nArcs >= 66433)
			thisLevel = thisLevel;
		
		tid = calcArcTimeCost(timeInt, i4, thisLevel, nextLevel, restrictedAreaNr, &calmWaterSpeed, fuelFactorMain); // , & fuel, & safety, & distance, & worstStormValue, & worstStabilityValue);
		//if (model.nArcs == 36039) {
	...
	if (tidInt <= max_t && tidInt >= min_t) {
		...
		if (thisLevel < 0 && nextLevel < 0) {
			...
			if (USE_KVOTCOST_CORRIDORS == 1)
				...
				if (thisLevel == nextLevel)
					kvotCost = model.network.channel[-thisLevel - 1].kvotMinCost *
					model.network.channel[-thisLevel - 1].minCost;
				else
					kvotCost = model.network.channel[-thisLevel - 1].kvotMinCost *
					model.network.channel[-thisLevel - 1].minCostOutLevel[-nextLevel - 1];
			}
		}
		else {
			channelCost = 0;
		...
		legNr = getLegNrFromLevels(thisLevel, nextLevel);
		safety = model.functions.valuesNow.worstStormValue *
			model.params.legWeights[legNr].weightSafety.hurricane +
			model.functions.valuesNow.bowSlam *
			model.params.legWeights[legNr].weightSafety.bowSlam +
			model.functions.valuesNow.greenWater *
			model.params.legWeights[legNr].weightSafety.greenWater +
			model.functions.valuesNow.dynamicStability *
			model.params.legWeights[legNr].weightSafety.dynamicStability +
			model.functions.valuesNow.rolling *
			model.params.legWeights[legNr].weightSafety.rolling +
			model.functions.valuesNow.surfRiding *
			model.params.legWeights[legNr].weightSafety.surfRiding +
			(1 - model.functions.valuesNow.feasibleSafety) *
			model.params.legWeights[legNr].weightSafety.feasibleSafety +
		...
		if (model.functions.valuesNow.maxWaveHeight > model.params.userLimit_maxWaveHeight)
			totCost += model.params.penOverWeatherLimit_fix +
			(model.functions.valuesNow.maxWaveHeight - model.params.userLimit_maxWaveHeight) * model.params.penOverMaxWaveHeightLimit_m;
		if (model.functions.valuesNow.maxWindSpeed > model.params.userLimit_maxWindSpeed_kmh)
			totCost += model.params.penOverWeatherLimit_fix +
			(model.functions.valuesNow.maxWindSpeed - model.params.userLimit_maxWindSpeed_kmh) * model.params.penOverMaxWindSpeedLimit_kmh;

		if (model.params.useSimulering == 1) {
			if (model.functions.valuesNow.maxWaveHeight > model.params.userSimulation_maxWaveHeight)
				totCost += model.simulering.penOverWeatherLimit_fix +
				(model.functions.valuesNow.maxWaveHeight - model.params.userSimulation_maxWaveHeight) * model.simulering.penOverMaxWaveHeight_m;
			if (model.functions.valuesNow.maxWindSpeed > model.params.userSimulation_maxWindSpeed_kmh)
				totCost += model.simulering.penOverWeatherLimit_fix +
				(model.functions.valuesNow.maxWindSpeed - model.params.userSimulation_maxWindSpeed_kmh) * model.simulering.penOverMaxWindSpeed_kmh;
			if (thisLevel >= 0) {
		...
		totCost += model.params.legWeights[legNr].weightTime * model.params.priceTime * tid +
			model.params.legWeights[legNr].weightFuel * fuelBase + model.params.legWeights[legNr].weightSafety.base * safety +
			emission * model.params.legWeights[legNr].weightEmission * model.params.scaleObjEmission;
		if (kvotCost < 0.99)
			kvotCost = kvotCost;
		...
		if (totCost < 0) { // } || model.nArcs == 2619560) {
			printf("\nchannelCost %.2lf wTime %.2lf pTime %.2lf tid %.2lf wFuel %.2lf fBase %.2lf emission %.2lf wEmission %.2lf wSafety %.2lf safety %.2lf totCost %.2lf nArcs %d\n",
				channelCost, model.params.legWeights[legNr].weightTime, model.params.priceTime, tid,
				1.0, fuelBase, emission, model.params.legWeights[legNr].weightEmission, model.params.legWeights[legNr].weightSafety.base, safety, totCost, model.nArcs);
			printf("worstStormVal %.2lf hurricaneWeight %.2lf feasibleSafety %.2lf weightFeasSafety %.2lf safety %.2lf weightSafety %.2lf\n",
				model.functions.valuesNow.worstStormValue, model.params.legWeights[legNr].weightSafety.hurricane,
				(1 - model.functions.valuesNow.feasibleSafety), model.params.legWeights[legNr].weightSafety.feasibleSafety, safety,
				model.params.legWeights[legNr].weightSafety.base);
	...
	double extraAreaCostKvot, double dist, double delayFactor)
{
	...
	if (thisLevel >= 0 || nextLevel >= 0) {
		...
		calmWaterSpeed = eval_calmWaterSpeed(i4, thisLevel, nextLevel, restrictedAreaNr);
		speedDiffCurrent = eval_speedDiffCurrent_delayedAlongArc(thisLevel, pos1, nextLevel, pos2, tPos, calmWaterSpeed);

		tid = calcDelayedArcTimeCost(thisLevel, nextLevel, restrictedAreaNr, i4, calmWaterSpeed, fuelFactorMain, delayFactor, dist, speedDiffCurrent);
		tidInt = timeInt + (int)round(tid * model.params.nTidsperioder_perH);
	}
	else {
		if (model.network.channel[-thisLevel - 1].timeThroughChannel > -0.5) {
		...
		nodNr1 = model.network.channel[-thisLevel - 1].nodNr_from_pt[pos1][tPos];
		timeInt = model.network.channel[-thisLevel - 1].timeInterval[pos1][tPos];
		tid = calcDelayedArcTimeCostChannel(timeInt, restrictedAreaNr, i4, -thisLevel - 1, delayFactor, &calmWaterSpeed);
		tidInt = model.network.channel[-thisLevel - 1].timeInterval[pos1][tPos] + (int)round(tid * model.params.nTidsperioder_perH);
	}
...
int genArcs_withDelay(int thisLevel, int pos1, int nextLevel, int pos2, int i2b, int tPos, int nSpeedSettings, double fuelQualityKvot,
	double extraAreaCostKvot, double* distArc, double delayFactor) {
	int i4, nArcs = 0, arcNr;

	int restrictedAreaNr = get_restrictedAreaNr(thisLevel, pos1, i2b);
	...
	for (i4 = 0; i4 < nSpeedSettings; i4++) {
		...
		arcNr = addEnBage_delayAB(thisLevel, pos1, nextLevel, pos2, i2b, restrictedAreaNr, tPos, i4, 0, 1e10, fuelQualityKvot, extraAreaCostKvot, *distArc, delayFactor);
		if (arcNr >= 0)
			nArcs++;
	}
	return nArcs;
	...
	if (posPrefP1 == pos1 && delayVersion == 1) {
		...
		if (thisLevel >= 0) {
			...
			if (*delayFactor < 0.0001) {
				...
				*delayFactor = eval_factorDelayedAlongPath(thisLevel, nextLevel, model.network.physicalLev[thisLevel].timeInterval[pos1][tPos], &speedDiffCurrent);
				if (nextLevel >= 0)
		...
		nArcs = genArcs_withDelay(thisLevel, pos1, nextLevel, pos2, i2b, tPos, nSpeedSettings, fuelQualityKvot, extraAreaCostKvot, distArc, *delayFactor);
	}
	else {
...
int determineBastSpeedDelay_routeToEnd_eta(strDelayToEnd* routeToEnd, double startTidp) {
	...
	int i, arcNr, cNr, nMaxChangeIter = 10, i0, nChange, restrictedAreaNr;
	...
	for (i0 = 0; i0 < nMaxChangeIter; i0++) {
		...
		for (i = 0; i < routeToEnd->nBVArcs; i++) {
			...
			if (timeDiff > 0) {
				...
				nSpeedSettings = determine_nSpeedSettingsToUse(modelDelay.arc[arcNr].fromLevel, 
					modelDelay.arc[arcNr].toLevel, restrictedAreaNr);
			...
			calmWaterSpeed = eval_calmWaterSpeed(speedSettingNu + model.delay.changedSpeed[i],
				modelDelay.arc[arcNr].fromLevel, modelDelay.arc[arcNr].toLevel, restrictedAreaNr);
...
int sparaLastWaypoint(FILE* filpekG, int* posReport, std::string solName) {
	...
	if(model.waypointResult.fromLevel >= 0)
		fprintf(filpekG, "    \"legPos\":%d,\n", model.network.physicalLev[model.waypointResult.fromLevel].legNr);
	else
		fprintf(filpekG, "    \"legPos\":%d,\n", model.network.channel[-model.waypointResult.fromLevel - 1].legNr);
	...
	fprintf(filpekG, "    \"accumDistance_km\":%.2lf, \"distanceLeft_nm\":%.1lf,\n", model.waypointResult.accumDistance_last_km,
		(model.functions.valuesNow.totDistance - model.waypointResult.accumDistance_last_km) / model.params.knots_to_km);
	...
	if (model.waypointResult.fromLevel == 10)
	...
	double fuelMainDay, fuelAuxDay;
	if (model.waypointResult.time_tot > 0.0001) {
		if (lev1 < 0 && lev2 < 0 && model.network.channel[-lev1 - 1].type != 1)
			fuelMainDay = (model.waypointResult.fuelMain_tot - model.waypointResult.fuelWaiting_main) * 24 / model.waypointResult.time_tot;
		else
			fuelMainDay = model.waypointResult.fuelMain_tot * 24 / model.waypointResult.time_tot;
	}else
		fuelMainDay = 0;
	if (model.waypointResult.hours > 0.0001) {
		if (lev1 < 0 && lev2 < 0 && model.network.channel[-lev1 - 1].type != 1)
			fuelAuxDay = (model.waypointResult.fuelAux_tot - model.waypointResult.fuelWaiting_aux) * 24 / model.waypointResult.time_tot;
		else
			fuelAuxDay = model.waypointResult.fuelAux_tot * 24 / model.waypointResult.hours;
	}else
		fuelAuxDay = 0;

	fprintf(filpekG, "    \"fuelConsumptionMain_ton_day\":%.5lf, \"fuelConsumptionAux_ton_day\":%.5lf, \"fuelECA_ton\":%.3lf\n",
		fuelMainDay, fuelAuxDay,
	...
	if ((lev1 >= 0 || lev2 >= 0) && model.waypointResult.speedSetting >= 0) {
		...
		if (model.waypointResult.waveHeight > model.params.userLimit_maxWaveHeight)
			fprintf(filpekG, "    \"waveHeight_level\":\"extreme\",\n");
		else if (model.waypointResult.waveHeight < model.params.maxWaveHeight_warning)
			fprintf(filpekG, "    \"waveHeight_level\":\"normal\",\n");
		else
			fprintf(filpekG, "    \"waveHeight_level\":\"high\",\n");
		if (model.waypointResult.windSpeed > model.params.userLimit_maxWindSpeed_kmh)
			fprintf(filpekG, "    \"windSpeed_level\":\"extreme\",\n");
		else if (model.waypointResult.windSpeed < model.params.maxWindSpeed_warning)
		...
		if (model.waypointResult.windReal_lastKnown < 5000) {
			...
			fprintf(filpekG, "    \"windSpeedReal_knots\":%.1lf, \"windDirection_degrees\":%.0lf, \"windDir_letters\":\"%s\",\n",
				0.0, 0.0, model.waypointResult.windDirReal_letters);
		}
		if (model.waypointResult.currentReal_lastKnown < 5000) {
		...
		if (model.waypointResult.waveDirReal_lastKnown < 5000) {
			...
			fprintf(filpekG, "    \"waveDirection_degrees\":%.0lf,\"waveDir_letters\":\"%s\",\n",
				0.0, model.waypointResult.waveDir_letters);
		}
	...
	model.functions.valuesNow.totWaitingTime += model.waypointResult.waitingTime;
...
int genSplitsArcNew2(int arcNr, int restrictedAreaNr, spherical::Point p1, spherical::Point p2, int prefPath) {
	double wantedTimeLength = 4.0, coordKvot;
	...
	double x, y, bearing, distTmp, calmWaterSpeed, distBas, tidWait, waitKvot;
	...
	if (model.arc[arcNr].fromLevel > 0)
		tidWait = model.network.physicalLev[model.arc[arcNr].fromLevel].tidWait;
	else
		tidWait = 0;
	if (model.arc[arcNr].time > 0.001 && tidWait > 0.001)
		waitKvot = tidWait / model.arc[arcNr].time;
	else
		waitKvot = 0;
	...
	if (prefPath == 1) {
		nWantedSplits = round((model.arc[arcNr].time - tidWait) / wantedTimeLength);
		...
		for (int i3 = startPos; i3 < endPos; i3++) {
			...
			for (ii = 0; ii < nWantedSplits + 1; ii++) {
				...
				if (distLastSplit + distTmp >= 1.7 * wantedDist && (distTot < distArc - 10 || distTmp > 0.5 * wantedDist)) {
					...
					model.network.posSplitCoord[nSplit] = model.network.nCoords;
					model.network.startKvot[nSplit] = waitKvot + (distTot - distTmp + wantedDist) / distArc * (1 - waitKvot);
			...
			if (distLastSplit >= wantedDist * 0.8 && distTot < distArc - wantedDist * 0.3) {
				model.network.posSplitCoord[nSplit] = model.network.nCoords;
				model.network.startKvot[nSplit] = waitKvot + distTot / distArc * (1 - waitKvot);
				model.network.endKvot[nSplit] = distTot;
		...
		for (i3b = 0; i3b < nSplit; i3b++) {
			if (i3b > 0)
				model.network.startKvot[i3b] = waitKvot + model.network.endKvot[i3b] / distTot * (1 - waitKvot);
			if (i3b > 0)
				model.network.endKvot[i3b - 1] = waitKvot + model.network.endKvot[i3b] / distTot * (1 - waitKvot);
		}
	...
	if (model.arc[arcNr].fromLevel < 0 && model.arc[arcNr].toLevel < 0 && model.arc[arcNr].fromLevel == model.arc[arcNr].toLevel) {
		...
		if (model.network.channel[cNr].timeThroughChannel > 0) {
			...
			if (model.network.nMaxSplits > 1) {
				calmWaterSpeed = eval_calmWaterSpeed(model.arc[arcNr].speedSetting, model.arc[arcNr].fromLevel, model.arc[arcNr].toLevel, restrictedAreaNr);
	...
	for (i = 0; i < nSplit; i++) {
		if (i == 0)
			model.network.startKvot[i] = 0.0;
		else {
			coordKvot = (double)i / nSplit;
			model.network.startKvot[i] = waitKvot + coordKvot * (1 - waitKvot);
		}
		model.network.endKvot[i] = waitKvot + (double)(i + 1) / nSplit * (1 - waitKvot);
		if (i > 0) {
			p3 = p1.destinationPoint(model.arc[arcNr].distance * 1000.0 * coordKvot, bearing);
			x = p3.longitude().degrees();
			y = p3.latitude().degrees();
		}
		else {
...
double getKvot_from_startEndWait(double arcTime, double tidWait, double startKvot, double endKvot, double* startKvot_dist, double* endKvot_dist) {
	double kvotUse = 0, kvot1, kvotWait, kvotDrive;
	*startKvot_dist = startKvot;
	*endKvot_dist = endKvot;

	if (arcTime > 0.001) {
		kvotWait = tidWait / arcTime;
		kvotDrive = 1 - kvotWait;
		if (kvotWait > startKvot)
			kvot1 = kvotWait;
		else
			kvot1 = startKvot;
		if (kvotDrive > 0.00001 && endKvot > kvot1) {
			kvotUse = (endKvot - kvot1) / kvotDrive;
			if (kvotDrive < 0.9999) {
				*endKvot_dist = (endKvot - kvotWait) / kvotDrive;
				if (startKvot > kvotWait)
					*startKvot_dist = (startKvot - kvotWait) / kvotDrive;
			}
		}
	}
	return kvotUse;
...
int addPositionDataToReport(FILE* filpekG, int* posReport, int arcNr, int startSlutArc, double* timeExact, std::string solName, int useFixCalmWaterSpeed, int iter) {
	...
	double fuelMan_tot, fuelAux_tot, time_tot, tidWait, kvotUse;
	struct tm tmBas = { 0 };
	tmBas.tm_isdst = 0;
	...
	tidWait = 0;
	if (lev1 >= 0) {
		tidWait = model.network.physicalLev[lev1].tidWait;
		...
		if (lev2 >= 0) {
			...
			if (lev1 == lev2) {
				...
				tidWait = model.network.channel[-lev1 - 1].waitingTime;
	...
	double timeCheck = model.arc[arcNr].time, accumTime = 0, startKvot, endKvot, startKvot_dist, endKvot_dist;
	int nSplit, ii, restrictedAreaNr;
	...
	if (lev2 < model.network.nPhysicalLevels) {
		...
		if (arcNr == 3)
			arcNr = arcNr;
		nSplit = genSplitsArcNew2(arcNr, restrictedAreaNr, p1, p2, prefPath);
	}
	...
	if (model.arc[arcNr].fromLevel == 79)
		arcNr = arcNr;
	if (arcNr == 39216)
		arcNr = arcNr;
	...
	if (model.arc[arcNr].fromTime * model.params.tIndexGerH >= model.network.tidp_startHistoricDataOnly) {
		...
		if (model.arc[arcNr].distance > 0.01)
			delayFactor = calmWaterSpeed * (model.arc[arcNr].time - tidWait) / model.arc[arcNr].distance;
		else
			delayFactor = 1.0;
		//}
		//else {
	...
	for (ii = 0; ii < nSplit; ii++) {
		...
		if (model.functions.valuesNow.sparaWaypointPos == -1)
			...
			if(model.functions.valuesNow.sparaWaypoint == 0) {
		...
		kvotUse = getKvot_from_startEndWait(model.arc[arcNr].time, tidWait, startKvot, endKvot, &startKvot_dist, &endKvot_dist);
		if (tidWait > startKvot * model.arc[arcNr].time)
			tidWait -= startKvot * model.arc[arcNr].time;
		else
			tidWait = 0;
		...
		if (model.arc[arcNr].fromTime * model.params.tIndexGerH >= model.network.tidp_startHistoricDataOnly) {
			...
			if(timeCheck > tidWait && model.arc[arcNr].time > 0){
				model.functions.valuesNow.fuel_main = (model.arc[arcNr].fuel_eca + model.arc[arcNr].fuel_noEca) * kvotUse;
		...
		*timeExact += timeCheck;
		...
		if (model.arc[arcNr].fromLevel == 79)
			arcNr = arcNr;
		if (timeCheck - tidWait > 0.01) {
			// speedOnGround = model.arc[arcNr].distance * (endKvot - startKvot) / timeCheck;
			speedOnGround = model.arc[arcNr].distance * kvotUse / (timeCheck - tidWait);
		}
		else
			speedOnGround = 0;
		...
		if (filpekG != NULL) {
			...
			if (model.functions.valuesNow.sparaWaypoint == 1) {
				...
				model.waypointResult.accumDistance_last_km = model.waypointResult.accumDistance_km;
			...
			if (startSlutArc == 0) {
				...
				if (kvotUse > 0.0001) {
					model.functions.valuesNow.accumDistance += model.arc[arcNr].distance * kvotUse;
					model.waypointResult.distance_nm += model.arc[arcNr].distance * kvotUse / model.params.knots_to_km;
				}
				...
				if (lev1 < 0 && lev2 < 0) {
					...
					if (timeCheck - tidWait > 0.001) {
						...
						fuel_day = (model.functions.valuesNow.fuel_main) * 24 / (timeCheck - tidWait);
						fuel_dayAux = (model.functions.valuesNow.fuel_aux) * 24 / timeCheck; // -tidWait);
						fuelMan_tot = model.functions.valuesNow.fuel_main;
						fuelAux_tot = model.functions.valuesNow.fuel_aux;
						time_tot = timeCheck - tidWait;
				...
				if ((lev1 >= 0 || lev2 >= 0) && model.arc[arcNr].speedSetting >= 0) {
					...
					model.functions.valuesNow.accumRPM += speedSetting.rpm[model.arc[arcNr].speedSetting] * (timeCheck - tidWait);
					model.functions.valuesNow.accumRPM_time += (timeCheck - tidWait);
					...
					model.functions.valuesNow.totTime_movingNoCorridors += timeCheck - tidWait;
					model.functions.valuesNow.sumSpeedOnWater += model.functions.valuesNow.speedOnWater * (timeCheck - tidWait);

					model.functions.valuesNow.totDistance_movingNoCorridors += model.arc[arcNr].distance * kvotUse;
					...
					if (printGlobal == 1)
						printf("arcNr %d levels %d %d wavePeriod %.3lf dist %.3lf time %.3lf speedOnGround %.3lf\n",
							...
							model.arc[arcNr].distance * kvotUse, model.arc[arcNr].time * (endKvot - startKvot), speedOnGround);
					...
					if (lev1 < 0 && lev2 < 0) {
						if (model.network.channel[-lev1 - 1].type == 1) { // tss
							model.functions.valuesNow.accumRPM += speedSetting.rpm[model.arc[arcNr].speedSetting] * (timeCheck - tidWait);
							model.functions.valuesNow.accumRPM_time += model.functions.valuesNow.accumRPM_time += (timeCheck - tidWait);
							if (model.functions.valuesNow.sparaWaypoint == 1) {
								model.waypointResult.current = model.functions.valuesNow.current / model.params.knots_to_km;
								model.waypointResult.speedSetting = model.arc[arcNr].speedSetting;
								model.waypointResult.calmWaterSpeed = model.functions.valuesNow.calmWaterSpeed / model.params.knots_to_km;
							...
							// model.functions.valuesNow.totWaitingTime += model.network.channel[-lev1 - 1].waitingTime;
						}
					}
					else {
						if (model.functions.valuesNow.sparaWaypoint == 1) {
				...
				if (lev1 >= 0 && tidWait > 0.001) {

					if (startKvot * model.arc[arcNr].time < tidWait) {
						if (endKvot * model.arc[arcNr].time <= tidWait)
							model.waypointResult.waitingTime += (endKvot - startKvot) * model.arc[arcNr].time;
						else
							model.waypointResult.waitingTime += tidWait;
					}
				}
				...
				if (model.functions.valuesNow.sparaWaypoint == 1) {
					if (model.functions.valuesNow.windReal < 5000) {
						...
						fprintf(filpekG, "    \"windSpeedReal_knots\":%.1lf, \"windDirection_degrees\":%.0lf, \"windDir_letters\":\"-\",\n",
							0.0, 0.0);
					if(model.functions.valuesNow.currentReal < 5000)
						fprintf(filpekG, "    \"currentReal_knots\":%.3lf, \"currentDirection_degrees\":%.0lf,\n",
							model.functions.valuesNow.currentReal / model.params.knots_to_km, model.functions.valuesNow.currentDirReal);
					else
						fprintf(filpekG, "    \"currentReal_knots\":%.3lf, \"currentDirection_degrees\":%.0lf,\n",
							model.functions.valuesNow.currentReal / model.params.knots_to_km, model.functions.valuesNow.currentDirReal);
					if (model.functions.valuesNow.waveDirReal < 5000) {
						fixDirectionLetters(model.functions.valuesNow.waveDirReal, startTime, 1);
						fprintf(filpekG, "    \"waveDirection_degrees\":%.0lf,\"waveDir_letters\":\"%s\",\n",
							model.functions.valuesNow.waveDirReal, startTime);
					}else
						fprintf(filpekG, "    \"waveDirection_degrees\":%.0lf,\"waveDir_letters\":\"-\",\n",
							0.0, startTime);
...
double evalWeatherDataAlongArcSection_equalTimeIntervals(int arcNr, int restrictedAreaNr, double startKvot, double* usedKvot, int startSlutArc, double timeExact, double delayFactor, int useFixCalmWaterSpeed) {
	...
	double stormVarde, uCurrent, vCurrent, deltaTid, tidWait, kvotUse;
	...
	int favorableWind, favorableWave, legNr;
	double tidTotArc, tidTotArc0, startKvot_dist, endKvot_dist;
	...
	if (arcNr >= 0) {
		if (model.arc[arcNr].toLevel < 0 && model.arc[arcNr].fromLevel < 0) {
			...
			if (startKvot < 0.001) {
				...
				tidWait = model.network.channel[cNr].waitingTime; // .intWaitingTime;
			}
		...
		kvotUse = getKvot_from_startEndWait(model.arc[arcNr].time, tidWait, startKvot, 1.0, &startKvot_dist, &endKvot_dist);
	}else
		kvotUse = getKvot_from_startEndWait(1, tidWait, startKvot, 1.0, &startKvot_dist, &endKvot_dist);
	if (tidWait > startKvot * model.arc[arcNr].time)
		tidWait -= startKvot * model.arc[arcNr].time;
	else
		tidWait = 0;
	...
	timeExact += tidWait;
	model.functions.valuesNow.waitingTime += model.network.physicalLev[model.arc[arcNr].fromLevel].tidWait;
	...
	dist = 0;
	distStart = model.arc[arcNr].distance * startKvot_dist;
	...
	if (startSlutArc == 0) {
		...
		if (USE_ARC_TIME_EXACT == 1)
			tidTotArc = model.arc[arcNr].fromTime * model.params.tIndexGerH + tidWait;
		else
			tidTotArc = tidTot;
		...
		for (i = 0; i < model.weatherFunctions.nCheckPoints; i++) {
			...
			if (fixTime <= 0) {
				...
				if (delayFactor < 0) {
					...
					tidTotArc += distNu / speedOverGround; // pfg 250503 should I have * useKvotNu;
					if (dist <= distStart) {
						continue;
					}
				...
				if (model.params.useSimulering == 1) {
					if (model.functions.valuesNow.maxWindSpeed < windSpeed) {
						if (windSpeed > model.params.userSimulation_maxWindSpeed_kmh && windSpeed > model.arc[arcNr].maxWindSpeed)
							windSpeed = model.arc[arcNr].maxWindSpeed;
					}
					if (model.functions.valuesNow.maxWaveHeight < waveHeight) {
						if (waveHeight > model.params.userSimulation_maxWaveHeight && waveHeight > model.arc[arcNr].maxWaveHeight)
				...
				fuelConsumption_main = eval_fuelConsumption_both(model.arc[arcNr].speedSetting, &fuelConsumption_aux,
					model.arc[arcNr].fromLevel, model.arc[arcNr].toLevel, restrictedAreaNr, calmWaterSpeed);
				if (arcNr == 22)
					arcNr = arcNr; // checkpfg
				...
				legNr = getLegNrFromLevels(model.arc[arcNr].fromLevel, model.arc[arcNr].toLevel);
				if (model.params.includeNazanin_safety >= 1)
					eval_safety_nazanin(legNr, speedOverGround, (calmWaterSpeed) - speedDiffWindWave, windSpeed, absWindDirDiff, waveHeight, wavePeriod, rel_waveDir, iceCover);
				else
					eval_safety(legNr, iceCover);
...
int resetValuesNow() {
	...
	model.results.maxWindSpeed_notAllowed = 0;
...
int addPositionDataToReport_equalTimeIntervals(FILE* filpekG, int* posReport, int arcNr, int startSlutArc, double* timeExact, std::string solName,
	...
	double x, y, bearing, speedOnGround, fuel_day, fuel_dayCheck, diff, diffTime, fuel_dayAux;
	double fuelQualityKvot = 0, kvotUse, startKvot_dist, endKvot_dist, tidWait;
	spherical::Point p1, p2, p3;
	...
	for (ii = 0; ii < nSplit; ii++) {
		...
		if (model.arc[arcNr].fromLevel == 87 && ii == 1)
			arcNr = arcNr;
		timeCheck = evalWeatherDataAlongArcSection_equalTimeIntervals(arcNr, restrictedAreaNr, startKvot, &usedKvot, startSlutArc, *timeExact, delayFactor, useFixCalmWaterSpeed) - (*timeExact);
		kvotUse = getKvot_from_startEndWait(model.arc[arcNr].time, tidWait, startKvot, usedKvot, &startKvot_dist, &endKvot_dist);
		if (timeCheck < 0.001)
			continue;
		...
		if (model.arc[arcNr].fromTime * model.params.tIndexGerH >= model.network.tidp_startHistoricDataOnly) {
			...
			model.functions.valuesNow.totFuel_main += (model.arc[arcNr].fuel_eca + model.arc[arcNr].fuel_noEca) * kvotUse;
		...
		*timeExact += timeCheck;
		...
		model.functions.valuesNow.distSinceLast += model.arc[arcNr].distance * kvotUse;
		model.functions.valuesNow.distSinceLastCalmWater += model.arc[arcNr].time * kvotUse * model.functions.valuesNow.calmWaterSpeed;
		...
		if (filpekG != NULL) {
			...
			if (model.arc[arcNr].fromLevel >= 0)
				fprintf(filpekG, "    \"legPos\":%d,\n", model.network.physicalLev[model.arc[arcNr].fromLevel].legNr);
			else
				fprintf(filpekG, "    \"legPos\":%d,\n", model.network.channel[-model.arc[arcNr].fromLevel - 1].legNr);
			...
			//	(model.functions.valuesNow.fuel_aux + model.functions.valuesNow.fuel_main) * (endKvot - startKvot) * (1 - fuelQualityKvot));
				...
				if ((lev1 >= 0 || lev2 >= 0) && model.arc[arcNr].speedSetting >= 0) {
					...
					fprintf(filpekG, "    \"waveHeight_m\":%.3lf, \"wavePeriod_s\":%.3lf,\n    \"relativeWaveDirection_degrees\":%.0lf,\n",
						model.functions.valuesNow.waveHeight, model.functions.valuesNow.wavePeriod,
						model.functions.valuesNow.relWaveDir);
					if (model.functions.valuesNow.waveHeight > model.params.userLimit_maxWaveHeight)
						fprintf(filpekG, "    \"waveHeight_level\":\"extreme\",\n");
					else if (model.functions.valuesNow.waveHeight < model.params.maxWaveHeight_warning)
						fprintf(filpekG, "    \"waveHeight_level\":\"normal\",\n");
					else
						fprintf(filpekG, "    \"waveHeight_level\":\"high\",\n");
					if (model.functions.valuesNow.windSpeed > model.params.userLimit_maxWindSpeed_kmh)
						fprintf(filpekG, "    \"windSpeed_level\":\"extreme\",\n");
					else if (model.functions.valuesNow.windSpeed < model.params.maxWindSpeed_warning)
						fprintf(filpekG, "    \"windSpeed_level\":\"normal\",\n");
					else
						fprintf(filpekG, "    \"windSpeed_level\":\"high\",\n");

					if (model.functions.valuesNow.windDirReal_lastKnown < 5000) {
					...
					if (printGlobal == 1)
						printf("arcNr %d levels %d %d wavePeriod %.3lf dist %.3lf time %.3lf speedOnGround %.3lf\n",
							arcNr, model.arc[arcNr].fromLevel, model.arc[arcNr].toLevel, model.functions.valuesNow.wavePeriod,
							model.arc[arcNr].distance * kvotUse, model.arc[arcNr].time * (usedKvot - startKvot), speedOnGround);
...
int makeSure_eta_inTime() {
	...
	int nSpeedsMax, restrictedAreaNr;
		...
		for (iPos = model.nBVArcs - 3; iPos >= -2; iPos--) {
			...
			if (fromLevel >= 0) {
				...
				if (baseSpeedNr < model.functions.nShip_speedSettingsBase - 1) {
					...
					restrictedAreaNr = get_restrictedAreaNr(fromLevel, modelDelay.arc[arcNr].fromPointNr,
						modelDelay.arc[arcNr].outNodePos);
					nSpeedsMax = determine_nSpeedSettingsToUse(fromLevel, model.arc[arcNr].toLevel, restrictedAreaNr);
					if (newSpeedNr >= nSpeedsMax && restrictedAreaNr >= 0) {
						newSpeedNr = nSpeedsMax - 1;
						speedNew = model.restrictedArea[restrictedAreaNr].max_speed;
					}
					deltaError -= timeArc[iPosUse] * (1 - speedOld / speedNew);
					if (newSpeedNr >= model.functions.speedLevel[fromLevel].nShip_speedSettings) {
						if (newSpeedNr >= model.functions.nAllocShipSpeedsLevel[model.network.physicalLev[fromLevel].legNr]) {
							nAlloc = newSpeedNr + 1;
...
int modify_nWaypoints(int iter) {
	int iPos, posDelay, arcNr, posNu, nAlloc, i, restrictedAreaNr;
	double timeExact;
	spherical::Point p1;
	...
	{
		arcNr = model.BVArc[iPos];
		if (model.arc[arcNr].fromLevel >= 21)
			arcNr = arcNr;
		if (model.arc[arcNr].speedSetting == -2) {
		...
		if (iPos != model.nBVArcs - 2) {
			model.waypoint[posNu].calmWaterSpeed = eval_calmWaterSpeed(model.arc[arcNr].speedSetting, model.arc[arcNr].fromLevel, model.arc[arcNr].toLevel, restrictedAreaNr);
			...
			if (model.arc[arcNr].fromLevel < 0) {
				...
				if (model.arc[arcNr].fromLevel > 0 &&
					model.network.physicalLev[model.arc[arcNr].fromLevel - 1].legNr != model.network.physicalLev[model.arc[arcNr].fromLevel].legNr)
					model.waypoint[posNu].type = 1;
			}
					...
					if (tidp - tpLast > tidDiff || i1 == i){
						...
						if (diff1 < diffPrev / 3 || i1 == i) {
							...
							if (diff1 / 3 > diffPrev)
								...
								if(model.waypoint[lastPos].use == 0)
...
{
	int nAllocPkter, i, iPos, nPkter, nArcs, ii3, forsta, speedSetting, legNr, restrictedAreaNr;
	...
	double totSafety = 0, totSafety_obj = 0;
	FILE* filRun;
	...
	double totMaxWindSpeed = 0;
	double timeNu2, fuel2, safety2, emissionNu;
	...
	double fuelCostDollar, fuel_objCost = 0, voyageTime_objCost = 0, emission_objCost = 0;
	...
	model.functions.valuesNow.totWaitingTime = 0;
	...
	for (iPos = 0; iPos < model.nBVArcs - 1; iPos++)
	{
		...
		if (model.arc[arcNr].speedSetting == -2) {
			...
			if (model.arc[arcNr].maxWaveHeight > model.params.userSimulation_maxWaveHeight)
				model.functions.valuesNow.obj_fel_maxWaveHeight += model.simulering.penOverWeatherLimit_fix +
				(model.arc[arcNr].maxWaveHeight - model.params.userSimulation_maxWaveHeight) * model.simulering.penOverMaxWaveHeight_m;
			if (model.arc[arcNr].maxWindSpeed > model.params.userSimulation_maxWindSpeed_kmh)
				model.functions.valuesNow.obj_fel_maxWindSpeed += model.simulering.penOverWeatherLimit_fix +
				(model.arc[arcNr].maxWindSpeed - model.params.userSimulation_maxWindSpeed_kmh) * model.simulering.penOverMaxWindSpeed_kmh;

			if (model.arc[arcNr].maxWaveHeight > model.params.userLimit_maxWaveHeight)
				model.functions.valuesNow.obj_fel_maxWaveHeight += model.params.penOverWeatherLimit_fix +
				(model.arc[arcNr].maxWaveHeight - model.params.userLimit_maxWaveHeight) * model.params.penOverMaxWaveHeightLimit_m;
			if (model.arc[arcNr].maxWindSpeed > model.params.userLimit_maxWindSpeed_kmh)
				model.functions.valuesNow.obj_fel_maxWindSpeed += model.params.penOverWeatherLimit_fix +
				(model.arc[arcNr].maxWindSpeed - model.params.userLimit_maxWindSpeed_kmh) * model.params.penOverMaxWindSpeedLimit_kmh;
		...
		costFuel += model.arc[arcNr].fuelBase;
		costTime += model.arc[arcNr].time * model.params.priceTime;
		...
		if (SKRIV_UT_NOTHING == 0) {
			...
			fprintf(filPek, "%d\t%d\t%d\t%.6lf\t%.6lf\t%.6lf\t%.6lf\t%.6lf\t%.6lf\t%.6lf\t%.6lf\t%.6lf\t%.6lf\t%.6lf\t%.6lf"
				...
				eval_calmWaterSpeed(model.arc[arcNr].speedSetting, model.arc[arcNr].fromLevel, model.arc[arcNr].toLevel, restrictedAreaNr) / model.params.knots_to_km,
		...
		if (lev1 < 0 && lev2 < 0) {
			...
			timeNu2 = model.arc[arcNr].time;
			timeNu += timeNu2;
			fuel2 = model.arc[arcNr].fuelBase;
			fuel += fuel2;
			safety2 = model.arc[arcNr].safetyBase;
			safety += safety2;
						...
						for (int i3 = 0; i3 < model.network.physicalLev[lev1].npreferredPathPoints; i3++) {
							...
							for (ii = 0; ii < nTp; ii++) {
								if (distNu + distTmp >= distanceTp || (i3 == model.network.physicalLev[lev1].npreferredPathPoints - 1 && distNu + distTmp >= distanceTp * 0.95)) {
									...
									timeNu2 += model.arc[arcNr].time / nTp;
									fuel2 += model.arc[arcNr].fuelBase / nTp;
									safety2 += model.arc[arcNr].safetyBase / nTp;
									if (safety > 100)
										safety = safety;
									hurricane += model.arc[arcNr].safetyHurricane / nTp;
									bowSlamming += model.functions.valuesNow.bowSlam / nTp; // model.arc[arcNr].safetyBowSlam / nTp;
		...
		emissionNu = model.functions.valuesNow.totFuel_aux * (1 - model.arc[arcNr].fuelQualityKvot) * model.params.fuel.aux_eca.emissionFactor +
		...
		if (resAlt == 0 || runAltForecast < 0) {
			fuel_objCost += model.params.legWeights[legNr].weightFuel * fuel2;
			voyageTime_objCost += model.params.legWeights[legNr].weightTime * timeNu2 * model.params.priceTime;
			emission_objCost += model.params.legWeights[legNr].weightEmission * emissionNu * model.params.scaleObjEmission;
		}
		else {
			fuel_objCost += model.params.extraOptWeights[resAlt - 1].weightFuel * fuel2;
			voyageTime_objCost += model.params.extraOptWeights[resAlt - 1].weightTime * timeNu2 * model.params.priceTime;
			emission_objCost += model.params.extraOptWeights[resAlt - 1].weightEmission * emissionNu * model.params.scaleObjEmission;
		}
		...
		safety = model.functions.valuesNow.worstStormValue *
			...
			model.functions.valuesNow.iceCoverCost;
		if (model.functions.valuesNow.maxWaveHeight > model.params.userLimit_maxWaveHeight)
			safety += model.params.penOverWeatherLimit_fix +
				(model.functions.valuesNow.maxWaveHeight - model.params.userLimit_maxWaveHeight) * model.params.penOverMaxWaveHeightLimit_m;
		if (model.functions.valuesNow.maxWindSpeed > model.params.userLimit_maxWindSpeed_kmh)
			safety += model.params.penOverWeatherLimit_fix +
				(model.functions.valuesNow.maxWindSpeed - model.params.userLimit_maxWindSpeed_kmh) * model.params.penOverMaxWindSpeedLimit_kmh;
		if (model.params.includeNazanin_safety >= 1) {
			...
			totGreenWater += model.functions.valuesNow.greenWater *
				model.params.legWeights[legNr].weightSafety.greenWater;
			if (model.functions.valuesNow.greenWater > 0.01)
				model.functions.valuesNow.greenWater = model.functions.valuesNow.greenWater;
			...
			totSafety_obj += model.arc[arcNr].safetyBase * model.params.legWeights[legNr].weightSafety.base;
			if (totSafety > 0)
				totSafety = totSafety;
		}
		else
	...
	if (timeNu - model.functions.valuesNow.totWaitingTime > 0)
		averSpeed = distance / (timeNu - model.functions.valuesNow.totWaitingTime) / model.params.knots_to_km;
	else
		averSpeed = 0;
	...
	if (model.params.includeNazanin_safety >= 1) {
		...
		if (iter == 2) {
			...
			sprintf(namn, "%s/resSafety_%s3.csv", model.params.resultPath.c_str(), namnSol);
			...
			if (sparaHeader == 1) {
				fprintf(filSafety, "caseID\tresID\thindCast\trunTime\tstartTime\tweightID\tweightTime\tweightFuel\tweightEmission\tweightsafetyBase\t"
					...
					"distTot_knots\tfuelTot_ton\ttimeTot_h\temissionTot\tdollarCostTimeFuel\tbowSlamming\tgreenWater\tdynStability\tparametricRolling"
				...
				fprintf(filSafety, "bowSlamMax\tbowSlamAver\tbowSlam_0\tbowSlam_01\tbowSlam_05\tbowSlam_2\t"
					...
					"stormValueMax\tstormValueAver\tdollarCostFuel\tdollarCostTime\tdollarCostExact_timeFuel\n");
			}
			fprintf(filSafety, "%s\t%s\t%d\t%s\t%s\t%d\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t", model.params.indataPathName.c_str(),
				...
				startTime0, model.params.weightID, model.params.legWeights[0].weightTime, model.params.legWeights[0].weightFuel,
				model.params.legWeights[0].weightEmission, model.params.legWeights[0].weightSafety.base,
				model.params.legWeights[0].weightSafety.bowSlam,
				model.params.legWeights[0].weightSafety.greenWater,
				model.params.legWeights[0].weightSafety.dynamicStability, model.params.legWeights[0].weightSafety.rolling, 
				model.params.legWeights[0].weightSafety.surfRiding,
				model.params.legWeights[0].weightSafety.hurricane);
			fprintf(filSafety, "%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t", distance / model.params.knots_to_km,
				...
				costFuel + costTime, // dollarCost + model.params.fuel.extra_fuel.quantity * model.params.fuel.extra_fuel.price,
				...
				totCost, totSafety_obj);

			if (nArcsUsed == 0)
				nArcsUsed = 1;
			fprintf(filSafety, "%lf\t%lf\t%lf\t%lf\t%lf\t",
				model.params.legWeights[0].weightSafety.bowSlam * model.results.bowSlam_aver * model.params.legWeights[0].weightSafety.base,
				model.params.legWeights[0].weightSafety.greenWater * model.results.greenWater_aver * model.params.legWeights[0].weightSafety.base,
				model.params.legWeights[0].weightSafety.dynamicStability * model.results.dynamicStability_aver * model.params.legWeights[0].weightSafety.base,
				model.params.legWeights[0].weightSafety.rolling * model.results.rolling_aver * model.params.legWeights[0].weightSafety.base,
				model.params.legWeights[0].weightSafety.surfRiding * model.results.surfRiding_aver * model.params.legWeights[0].weightSafety.base);
	...
	fprintf(filpekG, "\"totalSteamingTime_h\": %.1lf, \"totalAdditionalTime_h\": %.1lf,\n", timeNu - model.functions.valuesNow.totWaitingTime,
		model.functions.valuesNow.totWaitingTime); // timeNu);
	...
	if (model.results.fileForecast != NULL) {
		...
		fprintf(model.results.fileForecast, "weightTime;%lf;weightFuel;%lf;weightEmission;%lf;weightSafety;%lf\n",
			model.params.legWeights[0].weightTime, model.params.legWeights[0].weightFuel,
			model.params.legWeights[0].weightEmission, model.params.legWeights[0].weightSafety.base);
	...
	if (iter != 1) {
		fprintf(filpekG, "],\n");
		if (model.results.maxWaveHeight_notAllowed + model.results.maxWindSpeed_notAllowed < 0.01)
			fprintf(filpekG, "\"errorMessage\":\"OK\"\n}\n");
		else {
			fprintf(filpekG, "\"errorMessage\":\"A feasible path cannot be determined with the current constraints. Please modify the constraints and try again.\"\n}\n");
		}
	}
	...
	if (SKRIV_UT_NOTHING == 0) {
		...
		for (int i11 = 0; i11 < model.params.nLegs; i11++)
			fprintf(filPek, "%d\t%d\t%d\t%lf\t%lf\t%lf\n", i,
				...
				model.params.legWeights[i11].weightTime, model.params.legWeights[i11].weightFuel,
	...
	if (SPARA_RUN_DATA && iter == 2) {
		...
		fprintf(filRun, "%.2lf\t%.2lf\t%.2lf\t", timeNu - model.functions.valuesNow.totWaitingTime,
			model.functions.valuesNow.totWaitingTime,
			model.preferredPath.totDist / model.params.knots_to_km);
	...
	printf("dist\t%.2lf\ntime\t%.2lf\tcost\t%.2lf\tobj\t%.2lf\n"
		...
		fuel_aux* model.params.fuel.aux_noEca.price + fuel_auxEca * model.params.fuel.aux_eca.price + 
		fuel_eca * model.params.fuel.main_eca.price + fuel_noEca * model.params.fuel.main_noEca.price,
		fuel_objCost, emission, emission * model.params.scaleObjEmission, emission_objCost,
		channelCost, totSafety, totSafety_obj);
	if (model.params.includeNazanin_safety >= 1) {
		printf("    hurricane %.3lf\n    feasibleSafety %.3lf\n"
			"    ice %.3lf\n    maxWaveHeightCost %.3lf\n    maxWindSpeedCost %.3lf\n",
			totHurricane, totFeasSafety, totIce, totMaxWaveHeight, totMaxWindSpeed);
...
int identifyUsableFilDirsForecast() {
	...
	for (i = 0; i < 4; i++) {
		if (i == 0)
			...
			if (i == 3)
				path_name = model.params.weatherPath + "forecastData/202403/";
		}
...
int identCorrectForecastDir(int weatherNr, char* namn) {
	...
	if(model.filDirsForecast.presentPos >= 0)
		errlog("Last checked file %s.\n", namn);
...
int loadStaticWeatherFile_grib(int ii, int* nMaxTimeInt, int* initFile, char* namn, int* startPos, int* endPos, long long endTime_secondsUTC) {
	...
	if ((model.params.weather_is_current[ii] == 0 && model.params.ignore_windWave == 1) ||
		(model.params.weather_is_current[ii] == 1 && model.params.ignore_current == 1)) {
		model.weather[ii].nFiles = 1;
		model.weather[ii].filePos[0].minX = 0;
		model.weather[ii].filePos[0].maxX = 360;
		model.weather[ii].size_col = 180;
		model.weather[ii].size_row = 180;
		model.weather[ii].minX = 0;
		model.weather[ii].maxX = model.weather[ii].minX + 360;
		model.weather[ii].nCols = 2;
		model.weather[ii].maxY = 90;
		model.weather[ii].minY = model.weather[ii].maxY - 180;
		model.weather[ii].nRows = 1;
		nBands = 1;
		// nSecondsUTC = model.weather[ii].rasterPos[i1].GetSecondsFromUTC_metadataBand(i2 + 1);
		nBandsAlloc = (int)(nBands);
		model.weather[ii].secondsUTC = (long long*)malloc2(nBandsAlloc * sizeof(long long));
		model.weather[ii].valueCell = (float**)malloc2(nBandsAlloc * sizeof(float*));
		model.weather[ii].secondsUTC[0] = model.params.UTC_secondsStart + 3600 * 24 * 60;
		model.weather[ii].nTimeIntervals = 1;
		model.weather[ii].nTimeIntervals_forecast = 1;
		nAlloc = model.weather[ii].nCols * model.weather[ii].nRows;
		for (posNu = 0; posNu <= 0; posNu++) {
			model.weather[ii].valueCell[posNu] = (float*)malloc2(nAlloc * sizeof(float));
			for (int i3 = 0; i3 < nAlloc; i3++)
				model.weather[ii].valueCell[posNu][i3] = 0;
		}
...
int loadStorms_historical() {
	...
	if (model.params.ignore_windWave == 1)
		return 0; // no storms are used
...
int setNewSpeedAlt(int level1, strSpeed* speed, int baseSpeed) {
	int i, i1, posSave, indexUnder, indexOver;
	double maxSpeed, minSpeed, target, kvot;
	int legNr;

	if (level1 >= 0)
		legNr = model.network.physicalLev[level1].legNr;
	else
		legNr = model.network.channel[-level1 - 1].legNr;
	...
	if (iter == 1) {
		...
		for (int i = 0; i < model.network.nPhysicalLevels; i++) {
			...
			if (model.optPath.level[i].timeArrive >= -0.0001) {
				...
				setNewSpeedAlt(i, &(model.functions.speedLevel[i]), baseSpeed);
		...
		for (int i = 0; i < model.network.nChannels; i++) {
			if (model.network.channel[i].timeThroughChannel < 0) {
				if (model.optPath.channel[i].timeArriveThrough >= -0.0001) {
					...
					setNewSpeedAlt(-i - 1, &(model.functions.speedChannel[i]), baseSpeed);
				}
				if (model.optPath.channel[i].timeArriveNext >= -0.0001) {
					...
					setNewSpeedAlt(-i - 1, &(model.functions.speedChannelOut[i]), baseSpeed);
				}
			}
		}
	}
...
double evalEndTimeDelayAlongArc(double timeExact, int lev1, int lev2) {
	...
	if (lev1 >= 0 && lev2 >= 0) {
		delayFactor = eval_factorDelayedAlongPath(lev1, lev2, (int)timeExact, &speedDiff);
	...
	if (lev2 >= 0) {
		...
		delayFactor = eval_factorDelayedAlongPath(lev2 - 1, lev2, (int)timeExact, &speedDiff);
	}
	else
		pointNr2 = 0;
...
double evalCostArc(double tid, double fuelQualityKvot, double extraAreaCostKvot, int lev1, int lev2) {
	...
	int legNr = getLegNrFromLevels(lev1, lev2);
	safety = model.functions.valuesNow.worstStormValue *
		model.params.legWeights[legNr].weightSafety.hurricane +
		model.functions.valuesNow.bowSlam *
		model.params.legWeights[legNr].weightSafety.bowSlam +
...
double calcEndTime_withLowestCostSpeedAlongArc(int level1, int pointNr1, int level2, int pointNr2, double timeStart) {
	...
	double calmWaterSpeed = -1, kvotCost;
	if (USE_KVOTCOST_CORRIDORS == 1)
		kvotCost = 1.0;
	else
		kvotCost = 0.0;
	
	int restrictedAreaNr = get_restrictedAreaNr(level1, pointNr1, -1, level2, pointNr2);

	nSpeedSettings = determine_nSpeedSettingsToUse(level1, level2, restrictedAreaNr);
	if (level1 < 0 && level2 < 0){
		if (USE_KVOTCOST_CORRIDORS == 1)
			kvotCost = model.network.channel[-level1 - 1].kvotCost;
		else {
			if (level1 == level2)
				kvotCost = model.network.channel[-level1 - 1].kvotMinCost *
				model.network.channel[-level1 - 1].minCost;
			else
				kvotCost = model.network.channel[-level1 - 1].kvotMinCost *
				model.network.channel[-level1 - 1].minCostOutLevel[-level2 - 1];
		}
	}
	...
	for (ii = 0; ii < nSpeedSettings; ii++) {
		calmWaterSpeed = eval_calmWaterSpeed(ii, level1, level2, restrictedAreaNr);
		if (timeStart < model.network.tidp_startHistoricDataOnly) {
			// timeExact = evalWeatherDataAlongArc(-1, 0, timeExact);
			if (level1 >= 0 || level2 >= 0)
				tid = calcArcTimeCost(timeInt, ii, level1, level2, restrictedAreaNr, &calmWaterSpeed, -1);
			else
				tid = calcArcTimeCostChannel(timeInt, ii, -level1 - 1, restrictedAreaNr, &calmWaterSpeed);
		}
		else {
			if (ii == 0) {
				distArc = -1;
				if (level1 >= 0) {
					delayFactor = eval_factorDelayedAlongPath(level1, level2, timeInt, &speedDiffCurrent);
					if (level2 >= 0)
			...
			if (level1 >= 0 || level2 >= 0)
				...
				speedDiffCurrent = 0;
			tid = calcDelayedArcTimeCost(level1, level2, restrictedAreaNr, ii, calmWaterSpeed, -1, delayFactor, distArc, speedDiffCurrent);
			//timeExact = evalEndTimeDelayAlongArc(timeExact, i, -cNr - 1);
		}
		// costNu = evalCostArc(tid - timeStart, fuelQualityKvot, extraAreaCostKvot);
		costNu = evalCostArc(tid, fuelQualityKvot, extraAreaCostKvot, level1, level2); // *kvotCost;
		if (USE_KVOTKOST == 1) {
			if (USE_KVOTCOST_CORRIDORS == 1)
				costNu *= kvotCost;
			else
...
int evaluate_baseCost_corridors() {
	int i, i4, legNr, nSpeedSettings, i1, nextLevel, thisPos, nextPos, restrictedAreaNr;
	double extraAreaCostKvot, fuelQualityKvot, tid, calmWaterSpeed;
	double fuel_eca, fuel_noEca, fuel_aux, fuel_auxEca, fuelBase, emission, totCost, minCost;

	if (USE_KVOTCOST_CORRIDORS == 0 && USE_KVOTKOST == 1) {
		// loop through all channels and calculate an estimated cost for each if factorMinCost > 0
		for (i = 0; i < model.network.nChannels; i++) {
			model.network.channel[i].minCostOutLevel = (double*)calloc(model.network.nChannels, sizeof(double));
			if (model.network.channel[i].kvotMinCost > 0.00001) {
				for (i1 = -1; i1 < model.network.channel[i].nOutNodes; i1++) {
					if (i1 == -1) {
						nextLevel = -i - 1;
						thisPos = 0;
						nextPos = 1;
						...
						nextLevel = model.network.channel[i].outLevel[i1];
						if (nextLevel >= 0)
							continue; // only if connected to another channel
						thisPos = 1;
						nextPos = 0;
					}
					legNr = getLegNrFromLevels(-i - 1, nextLevel);
					restrictedAreaNr = get_restrictedAreaNr(-i - 1, nextLevel, i1);
					nSpeedSettings = determine_nSpeedSettingsToUse(-i - 1, nextLevel, restrictedAreaNr);
					if (model.network.channel[i].timeThroughChannel > -0.5)
						nSpeedSettings = 1; // only one speed option if fix speed through channel
					if(i1 == -1)
						calcWeatherPosAlongChannel(i); // , posPoly1, posPoly2);
					else
						calcWeatherPosAlongArc(model.network.channel[i].point[model.network.channel[i].nPoints - 1],
							model.network.channel[-nextLevel - 1].point[0], 0);
					extraAreaCostKvot = get_totalExtraAreaCostKvot(-i - 1, thisPos, nextLevel, nextPos, &fuelQualityKvot);
					minCost = 1e10;
					for (i4 = 0; i4 < nSpeedSettings; i4++) {
						if (i1 == -1)
							tid = calcArcTimeCostChannel(0, i4, i, restrictedAreaNr, &calmWaterSpeed);
						else
							tid = calcArcTimeCost(0, i4, -i - 1, nextLevel, restrictedAreaNr, &calmWaterSpeed, -1);
						fuel_eca = model.functions.valuesNow.fuel_main * (1 - fuelQualityKvot);
						fuel_noEca = model.functions.valuesNow.fuel_main * fuelQualityKvot;
						fuel_aux = model.functions.valuesNow.fuel_aux * fuelQualityKvot;
						fuel_auxEca = model.functions.valuesNow.fuel_aux * (1 - fuelQualityKvot);
						...
						totCost = model.params.legWeights[legNr].weightTime * model.params.priceTime * tid +
							model.params.legWeights[legNr].weightFuel * fuelBase +
							emission * model.params.legWeights[legNr].weightEmission * model.params.scaleObjEmission;
						totCost *= (1 + extraAreaCostKvot); // *kvotCost;
						if (totCost < minCost)
							minCost = totCost;
					...
					if(i1 == -1)
						model.network.channel[i].minCost = minCost;
					else
						model.network.channel[i].minCostOutLevel[-nextLevel - 1] = minCost;
				}
			}else
				model.network.channel[i].minCost = 0.0;
		}
	}
	return 0;
}
		...
		if (model.network.physicalLev[i].requirePrefPathFeasible == 1 || model.network.physicalLev[i + 1].requirePrefPathFeasible == 1) {
			...
			if (cNr >= 0) {
				...
				if (timeStart < model.network.tidp_startHistoricDataOnly) {
					...
					timeExact = evalWeatherDataAlongArc(-1, model.network.physicalLev[i].legNr, timeExact);
				}
				else {
					calcWeatherPosAlongArc(model.network.physicalLev[i].point[pointNr1], model.network.channel[cNr].point[0]);
				...
				timeExact = evalWeatherDataAlongArc(-cNr - 2, model.network.channel[cNr].legNr, timeExact);
				...
				if (timeStart < model.network.tidp_startHistoricDataOnly) {
					...
					timeExact = evalWeatherDataAlongArc(-1, model.network.channel[cNr].legNr, timeExact);
				}
				else {
					pointNr1 = model.params.preferredPathOrtoPos[iNext];
					if (pointNr1 < 0)
...
int get_restrictedAreaNr(int level1, int fromPointNr, int outNodePos, int level2, int toPointNr) {
	int areaNr, i, cNr;
	if (level1 >= 0) {
		if (level1 == model.network.nPhysicalLevels - 1)
			areaNr = -1;
		else {
			if (outNodePos >= 0)
				areaNr = model.network.physicalLev[level1].outRestrictedAreaNr[fromPointNr][outNodePos];
			else {
				for (i = 0; i < model.network.physicalLev[level1].nOutNodes[fromPointNr]; i++) {
					if (model.network.physicalLev[level1].outLevel[fromPointNr][i] == level2 &&
						model.network.physicalLev[level1].outNode[fromPointNr][i] == toPointNr)
						break;
				}
				if (i < model.network.physicalLev[level1].nOutNodes[fromPointNr])
					areaNr = model.network.physicalLev[level1].outRestrictedAreaNr[fromPointNr][i];
				else
					areaNr = -1;
			}
		}
...
int createTimeArcs(int runAlt)
{
	...
	int cNr, tidInt0, restrictedAreaNr;
	...
	for (i = 0; i < model.network.nPhysicalLevels - 1; i++) {
		...
		for (i1 = 0; i1 < model.network.physicalLev[i].nPoints; i1++) {
			...
			for (i2b = 0; i2b < model.network.physicalLev[i].nOutNodes[i1]; i2b++) {
				...
				if (runAlt == 1)
					i = i;
				addBagar_AB_speedSTid(i, i1, nextLevel, i2, i2b, &setupCheckPoints, min_t, max_t, fuelQualityKvot, extraAreaCostKvot, runAlt);
			}
		}
		...
		for (i1 = 0; i1 < model.network.nChannels; i1++) {
			...
			for (i2b = 0; i2b < model.network.channel[cNr].nOutNodes; i2b++) {
				...
				if (runAlt != 1)
					...
					get_minMax_timeFromLevel(-1, &min_t, &max_t);
				addBagar_AB_speedSTid(-cNr - 1, 1, nextLevel, model.network.channel[cNr].outNode[i2b], i2b, &setupCheckPoints,
	...
	for (i3 = 0; i3 < model.network.physicalLev[i].nTimeIntervals[i1]; i3++) {
		addEndBage(i, i1, i + 1, i3, nodNr2, 0);
		nArcsNu++;
	}
...
int genArcsToEnd_delayed(int thisLevel, int pos1, int nextLevel, int pos2, int i2b, int tPos, int nSpeedSettings)
{
	...
	int legNr = getLegNrFromLevels(thisLevel, nextLevel);
	totCost += model.params.legWeights[legNr].weightTime * model.params.priceTime * tid +
		model.params.legWeights[legNr].weightFuel * fuelBase + emission * model.params.legWeights[legNr].weightEmission * model.params.scaleObjEmission;
...
int addBagar_AB_speedSTid(int thisLevel, int pos1, int nextLevel, int pos2, int i2b, 
	int* setupCheckPoints, int min_t, int max_t,
	double fuelQualityKvot, double extraAreaCostKvot, int runAlt) {
	...
	if (thisLevel >= 0) {
		...
		for (i3 = 0; i3 < model.network.physicalLev[thisLevel].nTimeIntervals[pos1]; i3++) {
			if (model.network.physicalLev[thisLevel].timeInterval[pos1][i3] * model.params.tIndexGerH < model.network.tidp_startHistoricDataOnly) {
				for (i4 = 0; i4 < nSpeedSettings; i4++)
					arcNr = addEnBage_AB(thisLevel, pos1, nextLevel, pos2, i2b, restrictedAreaNr, i3, i4, 
						setupCheckPoints, min_t, max_t, fuelQualityKvot, extraAreaCostKvot, runAlt);
			}
			else {
				if (delayVersion < 3)
					arcNr = genArcsTo_delayedPreferredPath(thisLevel, pos1, nextLevel, pos2, i2b, i3, nSpeedSettings, fuelQualityKvot, extraAreaCostKvot, &delayFactor, &distArc);
				else {
					if (runAlt != 1) {// && model.results.onlyPrefPath_kaoutar != 1) {
						...
						arcNr = genArcsToEnd_delayed(thisLevel, pos1, nextLevel, pos2, i2b, i3, nSpeedSettings);
					}
					else
						arcNr = genArcsToEnd_delayed_prefPath(thisLevel, pos1, nextLevel, pos2, i2b, i3);
				}
			}
		}
	}
		...
		for (i3 = 0; i3 < model.network.channel[-thisLevel - 1].nTimeIntervals[1]; i3++) {
			...
			if (model.network.channel[-thisLevel - 1].timeInterval[pos1][i3] * model.params.tIndexGerH < model.network.tidp_startHistoricDataOnly) {
				for (i4 = 0; i4 < nSpeedSettings; i4++)
					arcNr = addEnBage_AB(thisLevel, pos1, nextLevel, pos2, i2b, restrictedAreaNr, i3, i4, setupCheckPoints, min_t, max_t, fuelQualityKvot, extraAreaCostKvot, runAlt);
		...
		if (thisLevel < 0 && nextLevel != thisLevel) {
			for (i3 = 0; i3 < model.network.channel[-thisLevel - 1].nTimeIntervals[1]; i3++) {
				...
				if (model.network.channel[-thisLevel - 1].timeInterval[1][i3] * model.params.tIndexGerH < model.network.tidp_startHistoricDataOnly) {
					for (i4 = 0; i4 < nSpeedSettings; i4++)
						arcNr = addEnBage_AB(thisLevel, 1, nextLevel, 0, i2b, restrictedAreaNr, i3, i4, setupCheckPoints, 0, 99999, fuelQualityKvot, extraAreaCostKvot, runAlt);
					if (USE_KVOTCOST_CORRIDORS == 1) { // this should not be needed with the new way 20250519
						kvotCost = model.network.channel[-thisLevel - 1].kvotCost;
						if (kvotCost < 1 && thisLevel == nextLevel) {
							minCost = 1e20;
					...
					if (runAlt != 1) {// && model.results.onlyPrefPath_kaoutar != 1)
						arcNr = genArcsToEnd_delayed(thisLevel, 1, nextLevel, 0, i2b, i3, nSpeedSettings);
					}
					else
						arcNr = genArcsToEnd_delayed_prefPath(thisLevel, 1, nextLevel, 0, i2b, i3);
				}
			}
		}
		else {
			...
			for (i3 = 0; i3 < model.network.channel[-nextLevel - 1].nTimeIntervals[0]; i3++) {
				...
				if (model.network.channel[-nextLevel - 1].timeInterval[0][i3] * model.params.tIndexGerH < model.network.tidp_startHistoricDataOnly) {
					...
					if (delayVersion < 3) {
						if (model.network.channel[-nextLevel - 1].timeThroughChannel <= -0.5) {
							delayFactor = eval_factorDelayedAlongPath(nextLevel, nextLevel, model.network.channel[-nextLevel - 1].timeInterval[0][i3], &speedDiffCurrent);
						}
						else
							delayFactor = 1;
		...
		for (i1 = 0; i1 < model.network.nChannels; i1++) {
			...
			for (i2b = 0; i2b < model.network.channel[cNr].nOutNodes; i2b++) {
				...
				if (runAlt != 1)
					...
					get_minMax_timeFromLevel(-1, &min_t, &max_t);
				addBagar_AB_speedSTid(-cNr - 1, 1, nextLevel, model.network.channel[cNr].outNode[i2b], i2b, 
					&setupCheckPoints, min_t, max_t, fuelQualityKvot, extraAreaCostKvot, runAlt);
			}
		}
...
int createArcsFromFixSol(FILE* fileFixSol, int runAlt)
{
	...
	for (i = 0; i < 10000; i++) {
		...
		setupCheckPoints = 1;
		restrictedAreaNr = get_restrictedAreaNr(fromLevel, fromPos, -1, toLevel, toPos);
		i3 = getTimeIntervalFromTidp(fromLevel, fromPos, toLevel, toPos, tidP);
		arcNr = addEnBage_AB(fromLevel, fromPos, toLevel, toPos, -1, restrictedAreaNr, i3, speedSetting, &setupCheckPoints, 0, toTime + 10000, fuelQualityKvot, extraAreaCostKvot, runAlt);
...
int createArcsFromFixSol2(int runAlt)
{
	...
	int antal, fromLevel, toLevel, fromPos, toPos, fromTime, toTime, speedSetting, tidP;
	int speedSettingBase, restrictedAreaNr;
	tidP = 0;
	nArcsTot = 0;
...
int genExtraOpts() {
	...
	printf("weightEmission %.2lf\nweightTime %.2lf\nweightFuel %.2lf\nweightSafetyBase %.2lf\neta_h %.2lf\n",
		model.params.legWeights[0].weightEmission, model.params.legWeights[0].weightTime, model.params.legWeights[0].weightFuel,
		model.params.legWeights[0].weightSafety.base, model.params.eta_h);

	// least$Cost
	if (model.params.legWeights[0].weightEmission > 0.15 || abs(model.params.legWeights[0].weightTime - 1) > 0.01 ||
		abs(model.params.legWeights[0].weightFuel - 1) > 0.01 || abs(model.params.legWeights[0].weightSafety.base - 0.1) > 0.1) {
	...
	// lowestEmission
	if (abs(model.params.legWeights[0].weightEmission - 1) > 0.01 || abs(model.params.legWeights[0].weightTime - 0.1) > 0.01 ||
		abs(model.params.legWeights[0].weightFuel - 0.1) > 0.01 || abs(model.params.legWeights[0].weightSafety.base - 0.1) > 0.1) {
	...
	// shortestPath
	if (model.params.legWeights[0].weightEmission > 0.01 || abs(model.params.legWeights[0].weightTime - 0.01) > 0.01 ||
		abs(model.params.legWeights[0].weightFuel - 0.01) > 0.01 || abs(model.params.legWeights[0].weightSafety.base - 0.01) > 0.1) {
	...
	if (model.params.eta_h > 0) {
		...
		model.params.extraOptWeights[nExtraOpt].weightEmission = model.params.legWeights[0].weightEmission;
		model.params.extraOptWeights[nExtraOpt].weightTime = model.params.legWeights[0].weightTime;
		model.params.extraOptWeights[nExtraOpt].weightFuel = model.params.legWeights[0].weightFuel;
		model.params.extraOptWeights[nExtraOpt].weightSafetyBase = model.params.legWeights[0].weightSafety.base;
...
int modify_utNodCost(int alt) {
	...
	for (i = 0; i < model.nNoder; i++) {
		for (i1 = 0; i1 < model.Noder[i].nUtNoder; i1++) {
			...
			if (alt <= 0) {
				legNr = getLegNrFromLevels(model.arc[arcNr].fromLevel, model.arc[arcNr].toLevel);
				weightTime = model.params.legWeights[legNr].weightTime;
				weightFuel = model.params.legWeights[legNr].weightFuel;
				weightSafety = model.params.legWeights[legNr].weightSafety.base;
				weightDistance = 0;
				weightEmission = model.params.legWeights[legNr].weightEmission;
			}
			if (arcNr == 74)
				arcNr = arcNr;
		...
		if (!dataTable["tableType"].is_null()) {
			...
			if (!dataTable["parameters"].is_null()) {
				...
				if (typeNr == 0) { // wind
					...
					if (!dataTable["maxWindSpeed"].is_null()) {
						model.tables.tableTyp[typeNr][pos].maxWindSpeed = (double)(dataTable["maxWindSpeed"]) * 1.852;
					}
					else {
						model.tables.tableTyp[typeNr][pos].maxWindSpeed = 41 * 1.852;
					}
				}
					...
					paramsError += readParameterInfoForTable(dataTable["parameters"], &(model.tables.tableTyp[typeNr][pos].windDirection), "windDirectionDiff", useFactor, M_PI / 180.0); // changed 250509
...
int identifyVesselSize(int* ballastLaden) {
	std::string resultat, storlek;
	std::string subNamn = "_:";
	size_t pos = std::string::npos, pos1, pos2;
	int size, storlekInt;

	resultat = model.functions.waveTableID;
	pos1 = resultat.find(subNamn);
	if (pos1 < 1000)
		pos2 = resultat.find(subNamn, pos1 + 1);
	else
		pos2 = 1000000;
	if (pos2 < 1000)
		pos = resultat.find(subNamn, pos2 + 1);
	else
		pos = 1000000;
	if (pos < 1000)
		pos = resultat.find(subNamn, pos + 1);
	
	if (pos2 < 1000) {
		storlek = resultat.substr(pos1 + 1, pos2 - pos1 - 1);
		storlekInt = atoi(storlek.c_str());
		if (storlekInt < 50000)
			size = 0;
		else {
			if (storlekInt < 120000)
				size = 1;
			else
				size = 2;
		}
		...
		errlog("ERROR! Failed to identify size of vessel from %s, I set it to medium\n", resultat.c_str());
		size = 1;
	}

	if (pos < 1000) {
		storlek = resultat.substr(pos + 1, 1);
		if (storlek == "B")
			*ballastLaden = 0;
		else {
			if (storlek == "L")
				*ballastLaden = 1;
				...
				errlog("ERROR! Failed to identify ballast/laden of vessel from %s, I set it to ballast, size was %s\n", 
					resultat.c_str(), storlek.c_str());
				*ballastLaden = 0;
			}
		}
	}
	else {
		errlog("ERROR! Failed to identify ballast/laden of vessel from %s, I set it to ballast\n", resultat.c_str());
		*ballastLaden = 0;
	}
...
int setSafetyTableIDsFromWaveTable() {
	int ballastLaden;
	std::string lastStorlek;

	int vesselSize = identifyVesselSize(&ballastLaden);
	if (ballastLaden == 0)
		lastStorlek = "ballast_";
	else
		lastStorlek = "laden_";
	if (vesselSize == 0)
		lastStorlek += "small";
	...
	model.functions.stabilityTableID = "dynamicStability_table_" + lastStorlek;
	model.functions.bowSlammingTableID = "bowSlamming_table_" + lastStorlek;
	model.functions.greenWaterTableID = "greenWater_table_" + lastStorlek;
	model.functions.rollingTableID = "rolling_table_" + lastStorlek;
	model.functions.surfRidingTableID = "surfRiding_table_" + lastStorlek;
	return 0;
}
	...
	for (int iter = 0; iter < 3; iter++) {
		...
		for (int ii = 0; ii < 1 + nExtraOpt; ii++) {
			...
			if (Reached == true) {
				...
				// dist = lasInLsngFromFil_MassTest(&model, Reached, nod1, nod2, &Cost); // use this to check a solution created by using saveArcInfoDijkstra == 1 in dijkstra.cpp 20250429
...
int voyageOpt_fixPartSol(std::string inputPath, std::string resultName)
{
	...
	for (int iter0 = 0; iter0 < model.iterKaoutar.nForecastRuns; iter0++) {
		...
		for(int legNr = 0; model.params.nLegs; legNr++)
			model.params.legCommercial[legNr].commercialAllowedVariation = -1.0;
...
int addBastSpeed_arcDelayed(int thisLevel, int pos1, int nextLevel, int pos2, int i2b, int tidInt, int nSpeedSettings, 
	...
	if (nextLevel >= 0) {
		if (thisLevel >= 0) {
			...
			waitingTime = model.network.physicalLev[thisLevel].tidWait;
		}
		else {
	...
	int legNr;
	if (thisLevel < 0)
		legNr = model.network.channel[-thisLevel - 1].legNr;
	else
		legNr = model.network.physicalLev[thisLevel].legNr;
		
	//for (i4 = 0; i4 < model.functions.nShip_speedSettingsDelay[legNr]; i4++) {
	int nShipSpeedSettings = determine_nSpeedSettingsToUse(thisLevel, nextLevel, restrictedAreaNr);
	for (i4 = 0; i4 < nShipSpeedSettings; i4++) {
		...
		if (fixTime < -0.5) {
			...
			calmWaterSpeed = eval_calmWaterSpeed(i4, -1, -1000, restrictedAreaNr);
			//else
			...
			//if (model.params.commercialAllowedVariation < 0)
			fuelConsumption_main = eval_fuelConsumption_both(i4, &fuelConsumption_aux, -1, -1000, restrictedAreaNr, calmWaterSpeed);
			//else
			...
			fuelConsumption_main = eval_fuelConsumption_both(i4, &fuelConsumption_aux, thisLevel, -1, restrictedAreaNr, calmWaterSpeed);
		...
		int legNr = getLegNrFromLevels(thisLevel, nextLevel);
		totCost += model.params.legWeights[legNr].weightTime * model.params.priceTime * timeArc +
			model.params.legWeights[legNr].weightFuel * fuelBase + emission * model.params.legWeights[legNr].weightEmission * model.params.scaleObjEmission;
	...
	if (addArc == 1) {
		...
		modelDelay.arc[arcNr].outNodePos = i2b;
		modelDelay.arc[arcNr].toPointNr = pos2;

		if (model.params.legCommercial[legNr].commercialAllowedVariation < 0)
...
int addBage_AB_delayFysiskt(int thisLevel, int pos1, int nextLevel, int pos2, int i2b, int tidInt, double fuelQualityKvot, double extraAreaCostKvot) {
	int nSpeedSettings, prefPath, i4, i3, arcNr;
	int nArcsNu = 0, restrictedAreaNr;
	double calmWaterSpeed = -1.0, distArc = -1, delayFactor = 0;
	double fuelFactorMain, nAddedTotArcs = 0, speedDiffCurrent = 0;

	restrictedAreaNr = get_restrictedAreaNr(thisLevel, pos1, i2b);
	nSpeedSettings = determine_nSpeedSettingsToUse(thisLevel, nextLevel, restrictedAreaNr);
	if (thisLevel >= 0) {
		if (pos1 == model.params.preferredPathOrtoPos[thisLevel]) {
			if (nextLevel >= 0) {
	...
	arcNr = addBastSpeed_arcDelayed(thisLevel, pos1, nextLevel, pos2, i2b, tidInt, nSpeedSettings, fuelQualityKvot, extraAreaCostKvot);
	// loop over all speedsettings, find lowest cost with delay factor
	...
	for (i = 0; i < model.network.nPhysicalLevels - 1; i++) {
		...
		for (i1 = 0; i1 < model.network.physicalLev[i].nPoints; i1++) {
			...
			for (i2b = 0; i2b < model.network.physicalLev[i].nOutNodes[i1]; i2b++) {
				...
				// fuelQualityKvot = get_fuelQualityKvot(i, i1, nextLevel, i2);
				extraAreaCostKvot = get_totalExtraAreaCostKvot(i, i1, nextLevel, i2, &fuelQualityKvot);
				addBage_AB_delayFysiskt(i, i1, nextLevel, i2, i2b, tidInt, fuelQualityKvot, extraAreaCostKvot);
			}
		}
		...
		for (i1 = 0; i1 < model.network.nChannels; i1++) {
			...
			//if (i < model.network.channel[cNr].earliestStartLevel || i > model.network.channel[cNr].latestEndLevel)
			//	continue;
			for (i2b = 0; i2b < model.network.channel[cNr].nOutNodes; i2b++) {
				nextLevel = model.network.channel[cNr].outLevel[i2b];
				if (nextLevel != i + 1 && nextLevel >= 0)
				...
				if (i == 2 && i1 == 3 && i2b == 8)
					i = i;
				addBage_AB_delayFysiskt(-cNr - 1, 1, nextLevel, model.network.channel[cNr].outNode[i2b], i2b, tidInt, fuelQualityKvot, extraAreaCostKvot);
			}
		}
...
int addBastSpeed_arcDelayed_prefPath(int thisLevel, int pos1, int nextLevel, int pos2, int i2b, int tidInt, double fuelQualityKvot, double extraAreaCostKvot) {
	// loop over all speedsettings, find lowest cost with delay factor
	int i4, minPos, posTmp;
	double minCost = 1e20, totCost, channelCost, calmWaterSpeed, timeArc;
	double fuelConsumption_main, fuelConsumption_aux, fuelUsage_main, fuelUsage_aux;
	...
	int restrictedAreaNr = get_restrictedAreaNr(thisLevel, pos1, i2b, nextLevel, pos2);
	...
	if (fixTime < -0.5) {
		...
		if (timeArc > 0.01)
			calmWaterSpeed = dist / timeArc;
		else
			calmWaterSpeed = 10;
	}
	fuelConsumption_main = eval_fuelConsumption_both(i4, &fuelConsumption_aux, -1, -100, restrictedAreaNr, calmWaterSpeed);
	...
	modelDelay_prefPath.arc[arcNr].outNodePos = i2b;
...
int solve_SP_delayPrefPath() {
	int i, i1, tidInt, nod1, i2b, i2, nextLevel, setupCheckPoints;
	int nAllocNoder, nAllocArcs, cNr, nAlloc, restrictedAreaNr;
	bool Reached;
	double fuelQualityKvot, extraAreaCostKvot;
	...
	for (i = 0; i < model.network.nPhysicalLevels - 1; i++) {
		...
		for (i1 = 0; i1 < model.network.physicalLev[i].nPoints; i1++) {
			...
			for (i2b = 0; i2b < model.network.physicalLev[i].nOutNodes[i1]; i2b++) {
				...
				// fuelQualityKvot = get_fuelQualityKvot(i, i1, nextLevel, i2);
				extraAreaCostKvot = get_totalExtraAreaCostKvot(i, i1, nextLevel, i2, &fuelQualityKvot);
				addBastSpeed_arcDelayed_prefPath(i, i1, nextLevel, i2, i2b, tidInt, fuelQualityKvot, extraAreaCostKvot);
			}
		}
		...
		for (i1 = 0; i1 < model.network.nChannels; i1++) {
			...
			for (i2b = 0; i2b < model.network.channel[cNr].nOutNodes; i2b++) {
				...
				// fuelQualityKvot = get_fuelQualityKvot(-cNr - 1, 1, nextLevel, model.network.channel[cNr].outNode[i2b]);
				extraAreaCostKvot = get_totalExtraAreaCostKvot(-cNr - 1, 1, nextLevel, model.network.channel[cNr].outNode[i2b], &fuelQualityKvot);
				addBastSpeed_arcDelayed_prefPath(-cNr - 1, 1, nextLevel, model.network.channel[cNr].outNode[i2b], i2b, tidInt, fuelQualityKvot, extraAreaCostKvot);
			}
		}
	...
	for (i1 = 0; i1 < model.network.nChannels; i1++) {
		...
		extraAreaCostKvot = get_totalExtraAreaCostKvot(-i1 - 1, 0, -i1 - 1, 1, &fuelQualityKvot);
		tidInt = (int)(round(model.network.channel[i1].midTimeArrive / model.params.tIndexGerH));
		addBastSpeed_arcDelayed_prefPath(-cNr - 1, 0, -cNr - 1, 1, -1, tidInt, fuelQualityKvot, extraAreaCostKvot);
	}
...
double evalWeatherDataAlongArc(int arcNr, int legNr, double timeExact){//, int speedSettingGiven) {
	int i, cNr, restrictedAreaNr;
	double windSpeed_x, windSpeed_y, waveDir_y, waveDir_x, dist, tidTot, distNu;
	...
	if (arcNr >= 0) {
		...
		if(model.arc[arcNr].fromLevel >= 0)
			timeExact += model.network.physicalLev[model.arc[arcNr].fromLevel].tidWait;
	}
	else {
		if (arcNr < -1) {
			cNr = -arcNr - 2;
	...
	if (fixTime > 0) {
		...
		if (arcNr >= 0) {
			restrictedAreaNr = get_restrictedAreaNr(model.arc[arcNr].fromLevel, model.arc[arcNr].fromPointNr,
				model.arc[arcNr].outNodePos);
			calmWaterSpeed = eval_calmWaterSpeed(model.arc[arcNr].speedSetting, model.arc[arcNr].fromLevel, 
				model.arc[arcNr].toLevel, restrictedAreaNr);
		}
		else {
			//if (speedSettingGiven >= 0)
			//	calmWaterSpeed = eval_calmWaterSpeed(speedSettingGiven, model.arc[arcNr].fromLevel, model.arc[arcNr].toLevel, restrictedAreaNr);
			//else
				calmWaterSpeed = model.params.preferredSpeed_calmWater;
		}
	}
	...
	for (i = 0; i < model.weatherFunctions.nCheckPoints; i++) {
		...
		//if(timeExact > 440)
		//	printf("stormVarde %lf worstVal %lf arcNr %d nArcs %d\n", stormVarde, model.functions.valuesNow.worstStormValue,
		//		arcNr, model.nArcs);
		if (stormVarde > model.functions.valuesNow.worstStormValue) {
			if(arcNr >= 0 && stormVarde > model.arc[arcNr].safetyHurricane)
				stormVarde = model.arc[arcNr].safetyHurricane; // to not create a high cost compared to initial arc generation
			model.functions.valuesNow.worstStormValue = stormVarde;
		}
...
int readCommercialData_leg (int legNr, json data, strParams* params) {

	if (!data["assigned_speed"].is_null())
		params->legCommercial[legNr].commercialSpeed = data["assigned_speed"];
	else
		params->legCommercial[legNr].commercialSpeed = -1;

	if (!data["assigned_consumption"].is_null())
		params->legCommercial[legNr].commercialFuel = data["assigned_consumption"];
	else
		params->legCommercial[legNr].commercialFuel = -1;

	if (!data["allowable_variation"].is_null())
		params->legCommercial[legNr].commercialAllowedVariation = data["allowable_variation"];
	else
		params->legCommercial[legNr].commercialAllowedVariation = 0;

	if (params->legCommercial[legNr].commercialSpeed <= 0 && params->legCommercial[legNr].commercialFuel <= 0) {
		if (params->legCommercial[legNr].commercialAllowedVariation > 0.001)
			errlog("ERROR! leg %d commercial allowable_variation is %.3lf but both assigned_speed and assigned_consumption is defined as not active\n",
				legNr + 1, params->legCommercial[legNr].commercialAllowedVariation);
		printf("commercial opt not used for leg %d\n", legNr + 1);
		params->legCommercial[legNr].commercialAllowedVariation = -1;
	}
	else {
		printf("commercial opt used\n");
		if (params->legCommercial[legNr].commercialSpeed > 0 && params->legCommercial[legNr].commercialFuel > 0) {
			errlog("ERROR! leg %d both commercial assigned_speed %.2lf and assigned_consumption %.2lf. Only one of them can be used, I use speed\n",
				legNr + 1, params->legCommercial[legNr].commercialSpeed, params->legCommercial[legNr].commercialFuel);
			params->legCommercial[legNr].commercialFuel = -1;
		}
		if (params->legCommercial[legNr].commercialAllowedVariation < 0.001) {
			if (params->legCommercial[legNr].commercialAllowedVariation < 0) {
				errlog("ERROR! leg %d commercial assigned_speed %.2lf and assigned_consumption %.2lf but allowable_variation is %lf. I set it to 0\n",
					legNr + 1, params->legCommercial[legNr].commercialSpeed, params->legCommercial[legNr].commercialFuel, 
					params->legCommercial[legNr].commercialAllowedVariation);
				params->legCommercial[legNr].commercialAllowedVariation = 0.0;
			}
			else
				errlog("OBS! leg %d commercial assigned_speed %.2lf and assigned_consumption %.2lf but allowable_variation is %lf.\n",
					legNr + 1, params->legCommercial[legNr].commercialSpeed, params->legCommercial[legNr].commercialFuel, 
					params->legCommercial[legNr].commercialAllowedVariation);
		}
	}
...
int readCommercialData(json data, strParams* params) {
	...
	if (data.is_array()) {
		for (auto it = data.begin(); it != data.end(); ++it) {
			dataFeature = it.value();
			readCommercialData_leg(legNr, dataFeature, params);
			legNr++;
		}
	}
	else
		readCommercialData_leg(legNr, data, params);
...
{ // not used
	...
	for (auto it = data2.begin(); it != data2.end(); ++it) {
		...
		if (!dataTable["properties"].is_null()) {
			...
			model.network.channel[pos].kvotMinCost = 0.0;
		...
		model.network.channel[pos].outRestrictedAreaNr = (int*)malloc2(sizeof(int));
		//model.network.channel[pos].outPolyPoint = (int*)malloc2(sizeof(int));
...
int load_geometry_onePolygon(strPolygonArea* polygon, json dataIt, int pos, int pos2) {
	int i0, i1, i2, nAlloc;
	i0 = 0;
	for (auto it1 = dataIt.begin(); it1 != dataIt.end(); ++it1) {
		if (i0 > 0) {
			free(polygon[pos].polygon[pos2].x);
			free(polygon[pos].polygon[pos2].y);
			postRequest("ERROR! More than one polygon in geometries for extraNoGo or restricted area " + std::string(polygon[pos].id) + ", fix the code to handle this. I skip this one for now.", 0);
			return -1;
		}
		json dataIt1 = it1.value();
		nAlloc = dataIt1.size();
		polygon[pos].polygon[pos2].x = (double*)malloc(nAlloc * sizeof(double));
		polygon[pos].polygon[pos2].y = (double*)malloc(nAlloc * sizeof(double));
		i1 = 0;
		for (auto it2 = dataIt1.begin(); it2 != dataIt1.end(); ++it2) {
			json dataIt2 = it2.value();
			i2 = 0;
			for (auto it3 = dataIt2.begin(); it3 != dataIt2.end(); ++it3) {
				if (i2 == 0) {
					polygon[pos].polygon[pos2].x[i1] = it3.value();
					//if (xVal > 180)
					//	xVal -= 360;
					//if (xVal < -180)
					//	xVal += 360;
				...
				if (i2 == 1)
					polygon[pos].polygon[pos2].y[i1] = it3.value();
				i2++;
			}
			i1++;
		}
		polygon[pos].polygon[pos2].nCoords = i1;
		i0++;
	}
	return 0;
}

int load_geometry(strPolygonArea* polygon, json dataGeo3, int pos) {
	int pos2, retVal;

	if (dataGeo3["type"].is_null()) {
		postRequest("ERROR! No type given for the geometry of extraNoGo or restricted area " + std::string(polygon[pos].id) + ".I skip this one!", 0);
		return -1;
	}
	std::string geoType = dataGeo3["type"];
	if (geoType != "Polygon" && geoType != "MultiPolygon") {
		postRequest("ERROR! Geometry type of extraNoGo or restricted area must be Polygon or MultiPolygon (but it is " + geoType + ").I skip this one!", 0);
		return -1;
	}

	if (dataGeo3["coordinates"].is_null()) {
		postRequest("ERROR! No coordinates given for the extraNoGo or restricted area " + std::string(polygon[pos].id) + ".I skip this one!", 0);
		return -1;
	}
	json dataCoord = dataGeo3["coordinates"];

	if (geoType == "MultiPolygon") {
		polygon[pos].nPolygons = dataCoord.size();
		polygon[pos].polygon = (strPolygon*)malloc(polygon[pos].nPolygons * sizeof(strPolygon));
		pos2 = 0;
		for (auto it = dataCoord.begin(); it != dataCoord.end(); ++it) {
			json dataIt = it.value();
			retVal = load_geometry_onePolygon(polygon, dataIt, pos, pos2);
			if (retVal < 0) {
				free(polygon[pos].polygon);
				return retVal;
			}
			pos2++;
		}
		...
		polygon[pos].nPolygons = 1;
		polygon[pos].polygon = (strPolygon*)malloc(polygon[pos].nPolygons * sizeof(strPolygon));
		retVal = load_geometry_onePolygon(polygon, dataCoord, pos, 0);
		if (retVal < 0) {
			free(polygon[pos].polygon);
			return retVal;
		}
...
double intersect_kvot(OGRLineString line, OGRPolygon* polygon) {
	double kvot = 0, line_dist;
	int skrivUtKoords = 0, p_inside[2], i, skrivUtAny = 0;

	for(i = 0; i < 2; i++)
		p_inside[i] = 0;

	for (i = 0; i < line.getNumPoints() && i < 2; i++) {
		OGRPoint point;
	...
	if (line.Intersects(polygon)) {
		...
		OGRGeometry* intersection = line.Intersection(polygon);
		...
		if (intersection != nullptr)
		{
			if (intersection->getGeometryType() == wkbPoint) {
				...
				kvot = 0.01; // only a point but want to avoid this
				skrivUtKoords = 1;
			}
				...
				kvot = intersectingLine->get_Length() / line_dist;
				// printf("kvot %.2lf line_dist %.2lf intersectLine %.2lf\n", kvot, line_dist, intersectingLine->get_Length());
				skrivUtKoords = 1;
			}
			else if (intersection->getGeometryType() == wkbMultiLineString) {
				OGRMultiLineString* multiIntersectingLine = (OGRMultiLineString*)intersection;
				// std::cout << "Intersection is a multiline string." << std::endl;
				for (int i0 = 0; i0 < multiIntersectingLine->getNumGeometries(); i0++) {
					OGRLineString* intersectingLine = (OGRLineString*)multiIntersectingLine->getGeometryRef(i0);
					if (skrivUtAny == 1) {
						for (int i = 0; i < intersectingLine->getNumPoints(); i++) {
							OGRPoint point;
							intersectingLine->getPoint(i, &point); //  &x, & y);
					...
					kvot += intersectingLine->get_Length() / line_dist;
					//printf("i0 %d kvot %.2lf line_dist %.2lf intersectLine %.2lf\n", i0, kvot, line_dist, intersectingLine->get_Length());
				}
				skrivUtKoords = 1;
			}
			else {
				std::cout << "Intersection is of an unknown type." << intersection->getGeometryType() << std::endl;
			}
	...
	if (skrivUtKoords == 10) {
		...
		std::cout << "Number of points in given line: " << numPoints << std::endl;
		...
		// Get the exterior ring (assuming it's a simple polygon)
		OGRLinearRing* exteriorRing = polygon->getExteriorRing();
		// Get the number of points in the ring
		numPoints = exteriorRing->getNumPoints();
		// Iterate through the points and print coordinates
		std::cout << "Polygon Coordinates:" << std::endl;
		for (int i = 0; i < numPoints; ++i) {
			OGRPoint point;
...
int testIntersect() {
	...
	lat1 = 48.666666666667;
	lon1 = 158;
	lat2 = 49.333333333333;
	lon2 = 158;
	lat1 = 49.3372;
	lon1 = 159.315;
	lat2 = 50;
	...
	OGRPolygon* polygon2 = new OGRPolygon();
	ring2->addPoint(48.423877992851317, 153.842366285326989);
	ring2->addPoint(51.757279339005137, 150.353923016096246);
	ring2->addPoint(55.788369339005101, 152.291947054557767);
	ring2->addPoint(54.082908185158956, 158.338582054557719);
	ring2->addPoint(49.664213377466687, 159.889001285326941);
	ring2->closeRings();
	polygon2->addRing(ring2);

	OGRPoint point1(lat1, lon1);
	OGRPoint point2(lat2, lon2);

	if (polygon2->Contains(&point1))
		printf("point1 in polygon\n");
	else
		printf("point1 NOT in polygon\n");
	if (polygon2->Contains(&point2))
		printf("point2 in polygon\n");
	else
		printf("point2 NOT in polygon\n");

	//line.addPoint(9, 6);

	double kvot = intersect_kvot(line, polygon2);
	printf("intersect kvot %lf\n", kvot);

	OGRLinearRing* ring3 = new OGRLinearRing();
	OGRPolygon* polygon3 = new OGRPolygon();
	ring3->addPoint(49.14673828,	159.369774);
	ring3->addPoint(49.39625891,	159.132366);
	ring3->addPoint(49.60459651,	159.3479712);
	ring3->addPoint(49.3478083,	159.5853792);
	ring3->addPoint(49.51980795,	159.7622239);
	ring3->addPoint(49.76448352,	159.5199709);
	ring3->addPoint(50.01642667,	159.7598014);
	ring3->addPoint(49.46651229,	159.9511813);
	ring3->closeRings();
	polygon3->addRing(ring3);

	kvot = intersect_kvot(line, polygon3);
	printf("intersect kvot %lf\n", kvot);
...
int testIntersect2() {
	...
	if (line.Intersects(polygon2)) {
		...
		OGRGeometry* intersection = line.Intersection(polygon2);
...
int add_custom_noGo_areas(json data) {
	int retVal, pos, i, i1, i2;
	std::string namnID;

	// testIntersect();

	//if (!data["customArea"].is_null()) {
	//	json dataExtra = data["customArea"];
	if (!data["features"].is_null()) {
		json dataExtra = data["features"];

		model.nExtraNoGoPolygons = dataExtra.size();
		model.extraNoGoPolygon = (strPolygonArea*)malloc(model.nExtraNoGoPolygons * sizeof(strPolygonArea));
		pos = 0;
		printf("nElement %d\n", dataExtra.size());
		for (auto it = dataExtra.begin(); it != dataExtra.end(); ++it) {
			...
			if (!dataNu["properties"].is_null()) {
				json dataProp = dataNu["properties"];
				if (!dataProp["customAreaID"].is_null()) {
					namnID = dataProp["customAreaID"];
				}
				else {
					namnID = "customArea_" + std::to_string(pos + 1);
					postRequest("ERROR! noGo_areas_polygons custom missing customAreaID, I set it to " + namnID, 0);
				}
			}
			else {
				namnID = "customArea_" + std::to_string(pos + 1);
				postRequest("ERROR! noGo_areas_polygons custom missing properties so no customAreaID, I set it to " + namnID, 0);
			}

			if (!dataNu["geometry"].is_null()) {
				model.extraNoGoPolygon[pos].id = str_alloc_cpy(namnID.c_str());
				retVal = load_geometry(model.extraNoGoPolygon, dataNu["geometry"], pos);
				if (retVal < 0)
					continue;
			}
			else {
				postRequest("ERROR! No geometry for noGo_areas_polygons " + namnID + ". I skip this one", 0);
				continue;
			}
			pos++;
		}
		model.nExtraNoGoPolygons = pos;
	}
	else {
		errlog("ERROR! noGo_areas_polygons given extraAreaID custom but no customArea in the object\n");
		postRequest("ERROR! noGo_areas_polygons given extraAreaID custom but no customArea in the object, I continue without custom areas", 0);
	}

	//int isOk;
	//	checkMinnesAnvandning(__LINE__);
	//	isOk = check_noGoPolygons_ok(-35.187, -9.579, -35.031, -11.27); // goes through nr 2
	//	isOk = check_noGoPolygons_ok(-35.164, -23.926, -35.204, -22.488); // goes through nr 1
	//	isOk = check_noGoPolygons_ok(-35.164, -23.926, -35.204, -23.564); // inside nr 1
	//	isOk = check_noGoPolygons_ok(-35.116, -37.596, -35.121, -38.72); // goes through nr 3
	//	isOk = check_noGoPolygons_ok(-35.116, -37.596, -35.121, -35.79); // does NOT goes through any of them
	//	checkMinnesAnvandning(__LINE__);

	//isOk = check_feasibleNode_noGo_polygons(-35.187, -9.579); // outside nr 2
	//isOk = check_feasibleNode_noGo_polygons(-35.031, -11.27); // inside nr 2
	//isOk = check_feasibleNode_noGo_polygons(-35.031, 348.73); // inside nr 2
	//isOk = check_feasibleNode_noGo_polygons(-35.031, -371.27); // inside nr 2
	//isOk = check_feasibleNode_noGo_polygons(-35.204, -22.488); // outside nr 1
	//isOk = check_feasibleNode_noGo_polygons(-35.164, -23.926); // inside nr 1
	//isOk = check_feasibleNode_noGo_polygons(-34.788, -38.26); // inside nr 3

	return 0;
}

int add_restricted_areas(json data) {
	int retVal, pos, i, i1, i2;
	std::string namnID;
	double speed, consumption;
	...
	if (!data["features"].is_null()) {
		...
		model.nRestrictedAreas = dataExtra.size();
		model.restrictedArea = (strPolygonArea*)malloc(model.nExtraNoGoPolygons * sizeof(strPolygonArea));
		pos = 0;
		printf("nElement %d\n", dataExtra.size());
		for (auto it = dataExtra.begin(); it != dataExtra.end(); ++it) {
			json dataNu = it.value();
			...
			if (!dataNu["properties"].is_null()) {
				json dataProp = dataNu["properties"];
				if (!dataProp["id"].is_null()) {
					namnID = dataProp["id"];
				}
				else {
					namnID = "restrictedArea_" + std::to_string(pos + 1);
				...
				if (!dataProp["main_fuelConsumptionMaxSpeed_mpd"].is_null()) {
					consumption = dataProp["main_fuelConsumptionMaxSpeed_mpd"];
				}
				else {
					postRequest("ERROR! restricted_area " + namnID + " missing fuel consumption, I use the tabled consumption", 0);
					consumption = -1;
				}
			}
			else {
				postRequest("ERROR! restricted_area missing properties so I skip it", 0);
				continue;
			}

			if (!dataNu["geometry"].is_null()) {
				retVal = load_geometry(model.restrictedArea, dataNu["geometry"], pos);
				if (retVal < 0)
					continue;
				model.restrictedArea[pos].id = str_alloc_cpy(namnID.c_str());
				model.restrictedArea[pos].max_speed = speed;
				model.restrictedArea[pos].main_fuelConsumption = consumption;
			}
			else {
				postRequest("ERROR! No geometry for restricted area " + namnID + ". I skip this one", 0);
				continue;
			}
			pos++;
		}
		model.nRestrictedAreas = pos;
	}
	else {
		errlog("ERROR! restricted_areas given but no features in the object\n");
		postRequest("ERROR! restricted_areas given but no features in the object, I continue without restricted areas", 0);
	}
...
{
	int i, closestI, nSplit, legNu, firstPoint;
	double xValOld, yValOld, last_x = -999, worstDegree, maxWind, diffI, diffNu;
	double fuelMain, fuelAux, xDiff, yDiff, xNu, yNu, dist, calmWaterSpeed;
	spherical::Point pNu;
	...
	params->includeNazanin_safety = 2;
#else
	...
	if (params->useSimulering == 1) {
		params->userSimulation_maxWaveHeight = 6.5; // 1e10;
		params->userSimulation_maxWindSpeed_kmh = 41 * 1.852; // 1e10;
		...
		if (!data["simulation_maxWaveHeight"].is_null())
			params->userSimulation_maxWaveHeight = data["simulation_maxWaveHeight"];
		if (!data["simulation_maxWindSpeed_kts"].is_null()) {
			params->userSimulation_maxWindSpeed_kmh = data["simulation_maxWindSpeed_kts"];
			params->userSimulation_maxWindSpeed_kmh *= 1.852;
		}
		//if (params->wayPointHours < 0)
		//	params->wayPointHours = 6; // want fix time way points when doing simulations
	}
	else {
		params->userSimulation_maxWaveHeight = 600.5; // 1e10;
		params->userSimulation_maxWindSpeed_kmh = 4100 * 1.852; // 1e10;
	}

	if (!data["limit_maxWaveHeight"].is_null())
		params->userLimit_maxWaveHeight = data["limit_maxWaveHeight"];
	else
		params->userLimit_maxWaveHeight = 1000;
	if (!data["limit_maxWindSpeed"].is_null()) {
		params->userLimit_maxWindSpeed_kmh = data["limit_maxWindSpeed"];
		params->userLimit_maxWindSpeed_kmh *= 1.852;
	}
	else
		params->userLimit_maxWindSpeed_kmh = 10000;

	if (params->userLimit_maxWaveHeight < 499 || params->userLimit_maxWindSpeed_kmh < 499 * 1.852) {
		params->nPkterOrto *= 1.5;
		if(2 * (int)(params->nPkterOrto / 2) == params->nPkterOrto)
			(params->nPkterOrto)++;
	}
	...
	if (!dataGeo["features"].is_null()) {
		...
		params->nLegs = dataGeo2.size();
		params->legWeights = (strLegWeights*)malloc(params->nLegs * sizeof(strLegWeights));
		params->legCommercial = (strLegCommercial*)malloc(params->nLegs * sizeof(strLegCommercial));

		for (i = 0; i < params->nLegs; i++) {
			params->legWeights[i].weightFuel = 1;
			params->legWeights[i].weightEmission = 1;
			params->legWeights[i].weightTime = 1;
			params->legWeights[i].weightSafety.base = 1000;
			params->legWeights[i].weightSafety.hurricane = 0;
			params->legWeights[i].weightSafety.bowSlam = 0;
			params->legWeights[i].weightSafety.greenWater = 0;
			params->legWeights[i].weightSafety.dynamicStability = 0;
			params->legWeights[i].weightSafety.rolling = 0;
			params->legWeights[i].weightSafety.surfRiding = 0;
			params->legWeights[i].weightSafety.feasibleSafety = 100000;
			params->legWeights[i].weightSafety.iceCoverCost_fix = 10000;
			params->legWeights[i].weightSafety.iceCoverCost_thickness = 0;
		}
		params->legProperties = (strLegProp*)malloc(params->nLegs * sizeof(strLegProp));
		i = 0;
		legNu = 0;
		xValOld = -999999;
		yValOld = -999999;
		for (auto it = dataGeo2.begin(); it != dataGeo2.end(); ++it) {
			params->legProperties[legNu].prefPath_startPoint = nPointsTot;
			firstPoint = 1;
			dataFeature = it.value();
			...
			if (namnNu != "preferredPath" && namnNu != "preferredPathFix" && namnNu != "optimizedPath") {
				printf("ERROR! Not the name preferredPath, preferredPathFix or optimizedPath of type for a property in geoData. I skip this one\n");
				errlog("ERROR! Not the name preferredPath, preferredPathFix or optimizedPath of type for a property in geoData. I skip this one\n");
				postRequest("ERROR! Not the type 'preferredPath', 'preferredPathFix' or 'optimizedPath' for property in geoData. It is '" + namnNu + "'. ", 0);
				continue; //not a preferred path
			}
			params->legProperties[legNu].path_fixed = 0;
			if (namnNu == "preferredPathFix")
				params->legProperties[legNu].path_fixed = 1;

			if (!dataProp["path_pos"].is_null()) {
				params->legProperties[legNu].path_pos = dataProp["path_pos"];
			}
			else
				params->legProperties[legNu].path_pos = -1;
			params->legProperties[legNu].endNode_exact = 0;
			if (!dataProp["endNodeType"].is_null()) {
				if (dataProp["endNodeType"] == "exact")
					params->legProperties[legNu].endNode_exact = 1;
			}
			params->legProperties[legNu].endNode_waitingTime = 0;
			if (!dataProp["endNode_waitingTime_h"].is_null()) {
				params->legProperties[legNu].endNode_waitingTime = dataProp["endNode_waitingTime_h"];
			}
			...
			if (geoType == "MultiLineString") {
					...
					for (auto it2 = dataIt.begin(); it2 != dataIt.end(); ++it2) {
						...
						if (abs(xVal - xValOld) > 0.00001 || abs(yVal - yValOld) > 0.00001) {
							...
							if (firstPoint == 1) {
								params->legProperties[legNu].prefPath_startPoint = nPointsTot;
								if (legNu > 0)
									params->legProperties[legNu - 1].prefPath_endPoint = nPointsTot - 1;
							}
							xValOld = xVal;
							yValOld = yVal;
							nPointsTot++;
				...
				for (auto it2 = dataCoord.begin(); it2 != dataCoord.end(); ++it2) {
					...
					if (abs(xVal - xValOld) > 0.00001 || abs(yVal - yValOld) > 0.00001) {
						...
						if (firstPoint == 1) {
							params->legProperties[legNu].prefPath_startPoint = nPointsTot - 1;
							if (legNu > 0)
								params->legProperties[legNu - 1].prefPath_endPoint = nPointsTot - 2;
						}
					}
					firstPoint = 0;
				}
			...
			params->legProperties[legNu].prefPath_endPoint = nPointsTot - 1;
			legNu++;
	...
	if (!data["commercial"].is_null())
		...
		for (int legNr = 0; legNr < params->nLegs; legNr++) {
			params->legCommercial[legNr].commercialSpeed = -1;
			params->legCommercial[legNr].commercialFuel = -1;
			params->legCommercial[legNr].commercialAllowedVariation = -1;
		}
	...
	if (!data["restricted_areas"].is_null()) {
		dataGeo = data["restricted_areas"];
		add_restricted_areas(dataGeo);
	}
	else
		model.nRestrictedAreas = 0;
	...
	if (!data["ship_specification"].is_null()) {
		...
		calmWaterSpeed = eval_calmWaterSpeed(model.functions.speedSetting95MCR_base, -1, -100, -1);
		fuelMain = eval_fuelConsumption_both(model.functions.speedSetting95MCR_base, &fuelAux, -1, -100, -1, calmWaterSpeed);
		errlog("max rpm gives a calmWaterSpeed of %.2lf knots and a fuel consumption of (main/aux) %.2lf %.2lf ton/hour\n",
			calmWaterSpeed / params->knots_to_km, fuelMain, fuelAux);
...
*/
	...
	if (model.params.includeNazanin_safety == 1) {
		setSafetyTableIDsFromWaveTable();
	}
...
//	dataGeo = data["geoData"];
	...
	errlog("nPoints in preferredPath %d from %d legs\n", model.preferredPath.nPoints, model.params.nLegs);
	...
	if (params->useSimulering == 1) {
		for (i = 0; i < params->nLegs; i++) {
			params->legWeights[i].weightEmission = 0;
			params->legWeights[i].weightFuel = 0;
			params->legWeights[i].weightTime = 0;
		}
	}
...
int loadAllNeededTablesFromSQLite() {
	...
	for (int ii = 0; ii < nTables; ii++) {
		if (ii == 0) { // wind
			...
			if(model.params.userLimit_maxWindSpeed_kmh > 9999)
			...
			if(model.params.userLimit_maxWaveHeight > 999)
...
double lookup_dynamicStabilityTable(double windSpeed, double relWindDirection, double shipSpeed) {
	...
	if (windSpeed < model.functions.dynStability.windSpeed.minValue)
		return 0.0; // no unstability if no wind
	...
	shipSpeedIndex = get_tableIndex(shipSpeed, model.functions.dynStability.shipSpeedOverGround); // getWindDirectionIndex(relWindDirection, model.functions.dynStability);
...
double lookup_surfRidingTable(double waveHeight, double wavePeriod, double relWaveDirection, double shipSpeed) {
	...
	dirIndex = get_tableIndexDirection(relWaveDirection, model.functions.surfRiding.waveDirection);
...
double lookup_rollingTable(double waveHeight, double wavePeriod, double relWaveDirection, double shipSpeed) {
	...
	dirIndex = get_tableIndexDirection(relWaveDirection, model.functions.rolling.waveDirection);
	//shipSpeedIndex = get_tableIndexDirection(shipSpeed, model.functions.rolling.relShipSpeed);
	shipSpeedIndex = get_tableIndex(shipSpeed, model.functions.rolling.relShipSpeed);
...
void eval_safety_nazanin(int legNr, double shipSpeedOverLand, double shipSpeedRelWater, double windspeed, double absWindDirDiff, double waveHeight,
	...
	if (bowSlam > 0.999 && model.params.legWeights[legNr].weightSafety.bowSlam > 0.0001) {
	...
	if (greenWater > 0.999 && model.params.legWeights[legNr].weightSafety.greenWater > 0.0001)
		model.functions.valuesNow.feasibleSafety = 0;

	if (model.functions.valuesNow.rolling < rolling)
		model.functions.valuesNow.rolling = rolling;
	if (rolling > 0.999 && model.params.legWeights[legNr].weightSafety.rolling > 0.0001)
		model.functions.valuesNow.feasibleSafety = 0;
	...
	if (surfRiding > 0.999 && model.params.legWeights[legNr].weightSafety.surfRiding > 0.0001)
		model.functions.valuesNow.feasibleSafety = 0;

	if (model.functions.valuesNow.dynamicStability < dynStab)
		model.functions.valuesNow.dynamicStability = dynStab;
	if (dynStab >= 0.999 && model.params.legWeights[legNr].weightSafety.dynamicStability > 0.0001)
		model.functions.valuesNow.feasibleSafety = 0;


	if (iceCover > model.functions.iceCoverMaxFree) {
		iceCost = model.params.legWeights[legNr].weightSafety.iceCoverCost_fix + model.params.legWeights[legNr].weightSafety.iceCoverCost_thickness * (
...
int getAllVariableValues(int checkPointNr, double tidpkt)
 {
	...
	for (varNr = 0; varNr < model.nWeatherFiles; varNr++) {
		...
		if (model.functions.varValue[varNr] < -1.0e10 || model.functions.varValue[varNr] > 1.0e10) {
			errlog("Error! varNr %d tidpkt %lf tidInt %d tidIndex %d posLatLon %d latPos %d lonPos %d value %lf lat/lon %lf %lf\n", varNr, tidpkt, tidInt, tidIndex,
				model.weatherFunctions.checkPoint[checkPointNr].pos_latLon[varNr],
				model.weatherFunctions.checkPoint[checkPointNr].latPos[varNr], model.weatherFunctions.checkPoint[checkPointNr].lonPos[varNr],
				model.functions.varValue[varNr],
				model.weather[varNr].maxY - model.weatherFunctions.checkPoint[checkPointNr].latPos[varNr] * model.weather[varNr].size_row,
				model.weather[varNr].minX + model.weatherFunctions.checkPoint[checkPointNr].lonPos[varNr] * model.weather[varNr].size_col);
			model.functions.varValue[varNr] = 0;
		}
...
double evalWeatherDataAlongArcSection(int arcNr, double startKvot, double endKvot, int startSlutArc, double timeExact, double delayFactor, int useFixCalmWaterSpeed) {
	int i, cNr, tidInt, delayNr, tidIntForecast, restrictedAreaNr;
	double checkFactor, speedNu, calcDelayFactor, checkSpeedDiffCurrent, tidNu, tidCalmWater;
	double windSpeed_x, windSpeed_y, waveDir_y, waveDir_x, dist, tidTot, distNu;
	double stormVarde, uCurrent, vCurrent, deltaTid;
	double currentReal_u = 0, currentReal_v = 0, windSpeedReal_u = 0, windSpeedReal_v = 0, waveHeightReal_u = 0, waveHeightReal_v = 0;
	...
	double fixTime = -1, distStart, distEnd, useKvotNu, kvotBort;
	double waveHeightBase, waveDirectionBase, fuelTot_main = 0, fuelTot_aux = 0;
	double timeArcSTW, absWindDirDiff = 0, startKvot_dist, endKvot_dist;
	int favorableWind, favorableWave, legNr;
	double tidTotArc, tidTotArc0, tidWait, kvotUse, kvotWait, kvot1, kvot2, tidWait_init;
	...
	if (arcNr >= 0) {
		if (model.arc[arcNr].toLevel < 0 && model.arc[arcNr].fromLevel < 0) {
			...
			//if (startKvot < 0.001) {
				...
				tidWait = model.network.channel[cNr].waitingTime; // .intWaitingTime;
			//}
				if (model.network.channel[cNr].timeThroughChannel > -0.5) {
					fixTime = model.network.channel[cNr].timeThroughChannel; // *(endKvot - startKvot));
					//kvotUse = getKvot_from_startEndWait(fixTime + tidWait, tidWait, startKvot, endKvot);
				}
		}
		else {
			if (model.arc[arcNr].speedSetting < 0)
		...
		kvotUse = getKvot_from_startEndWait(1, tidWait, startKvot, endKvot, &startKvot_dist, &endKvot_dist);
	tidWait_init = tidWait;
	if (tidWait > startKvot * model.arc[arcNr].time)
		tidWait -= startKvot * model.arc[arcNr].time;
	else
		tidWait = 0;
	...
	dist = 0;
	distStart = model.arc[arcNr].distance * startKvot_dist;
	distEnd = model.arc[arcNr].distance * endKvot_dist;
	...
	if (startSlutArc == 0) {
		...
		if (USE_ARC_TIME_EXACT == 1)
			tidTotArc = model.arc[arcNr].fromTime * model.params.tIndexGerH + tidWait_init;
		else
			tidTotArc = tidTot;
		tidTotArc0 = tidTotArc;
		...
		for (i = 0; i < model.weatherFunctions.nCheckPoints; i++) {
			...
			if (fixTime <= 0) {
				...
				if (delayFactor < 0) {
					...
					tidTotArc += distNu / calmWaterSpeed * delayFactor; // pfg 250503 should I have * useKvotNu;
					if (dist <= distStart) {
						continue;
					}
				...
				if (model.functions.valuesNow.maxWindSpeed < windSpeed) {
					if (windSpeed > model.params.userLimit_maxWindSpeed_kmh && windSpeed > model.arc[arcNr].maxWindSpeed)
						windSpeed = model.arc[arcNr].maxWindSpeed;
				}
				if (model.functions.valuesNow.maxWaveHeight < waveHeight) {
					if (waveHeight > model.params.userLimit_maxWaveHeight && waveHeight > model.arc[arcNr].maxWaveHeight)
						waveHeight = model.arc[arcNr].maxWaveHeight;
				}
				if (model.functions.valuesNow.maxWindSpeed < windSpeed &&
					model.weather[model.functions.pos_wind_u].timeIntervalIndex[tidIntForecast] < model.weather[model.functions.pos_wind_u].nTimeIntervals_forecast) {
				...
				if (arcNr == 22)
					arcNr = arcNr; // checkpfg
				fuelUsage_main = fuelConsumption_main * timeArc;
				fuelUsage_aux = fuelConsumption_aux * (timeArc + tidWait);
				tidWait = 0;
				if (model.arc[arcNr].fromLevel < 0 && model.arc[arcNr].toLevel < 0) {
