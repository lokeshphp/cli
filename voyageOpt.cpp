...
int writeKorridorToGeojson(char* pszFilename)
{
	...
	x = (double*)malloc2(model.network.nPhysicalLevels * sizeof(double));
	y = (double*)malloc2(model.network.nPhysicalLevels * sizeof(double));
	z = (double*)malloc2(model.network.nPhysicalLevels * sizeof(double));
	...
	if (startSlutArc == 0) {
		...
		for (i = 0; i < model.weatherFunctions.nCheckPoints; i++) {
			...
			if (fixTime <= 0) {
				...
				windSpeed_x += timeArc * rel_windSpeed * cos(rel_windDir);
				windSpeed_y += timeArc * rel_windSpeed * sin(rel_windDir);
	...
	if (startSlutArc == 0) {
		...
		if (fixTime > 0) {
			if(fixTime < 9999)
				...
				if (useFixCalmWaterSpeed == 0)
					...
					calmWaterSpeed = model.params.calmWaterSpeedCompare;
			}
		...
		for (i = 0; i < model.weatherFunctions.nCheckPoints; i++) {
			...
			if (fixTime <= 0) {
				...
				if (tidTot < model.network.tidp_startHistoricDataOnly) {
					uCurrent = model.functions.varValue[model.functions.pos_current_u]; // getVariableValue(model.functions.pos_current_u, i, tidTot);
					vCurrent = model.functions.varValue[model.functions.pos_current_v]; // getVariableValue(model.functions.pos_current_v, i, tidTot);
				}
				else {
				...
				if (delayFactor < 0) {
					...
					tidInt = (int)tidTot / model.params.tIndexGerH;
					...
					//if (SKRIV_UT_NOTHING == 0) {
					//	errlog("bbb %d %lf %.2lf %d %lf %lf %lf %lf"
				...
				model.functions.valuesNow.sumWaveHight += waveHeight * timeArc;
				...
				if (model.functions.valuesNow.maxWaveHight < waveHeight)
					model.functions.valuesNow.maxWaveHight = waveHeight;
		...
		*timeExact += timeCheck;
		...
		if (filpekG != NULL) {
			...
			if (startSlutArc == 0) {
				...
				fprintf(filpekG, "    \"fuelConsumptionMain_ton_day\":%.5lf, \"fuelECA_ton\":%.3lf\n",
					fuel_day,
				...
				if ((lev1 >= 0 || lev2 >= 0) && model.arc[arcNr].speedSetting >= 0) {
					...
					if (model.functions.valuesNow.windReal < 1000 || model.functions.valuesNow.windDirReal_lastKnown < 1000) {
						...
						fprintf(filpekG, "    \"windSpeedReal_knots\":%.1lf, \"windDirection_degrees\":%.0lf, \"windDir_letters\":\"%s\",\n",
							model.functions.valuesNow.windReal_lastKnown / model.params.knots_to_km, model.functions.valuesNow.windDirReal, startTime);
					}
				...
				fprintf(filpekG, "    \"speedSetting\":%d, \"speedCalmWater_kts\":%.1lf, \"speedOnGround_kts\":%.1lf, \"rpm\":%.2lf,\n",
					-1, 0, 0, 0);
				fprintf(filpekG, "    \"fuelConsumptionMain_ton_day\":%.5lf, \"fuelECA_ton\":%.3lf, \"current_kts\":%.1lf,\n",
...
double calcNewTime_changeSpeedSetting_delay_prefPath(int arcDelay, int speedSettingNu, int tidInt) {
	//double calcSpeed = modelDelay.arc[arcDelay].distance / modelDelay.arc[arcDelay].time;
	//double calmWaterSpeed = eval_calmWaterSpeed(modelDelay.arc[arcDelay].speedSetting, -1, -100);
	//double factor = calmWaterSpeed / calcSpeed;
	double calmWaterSpeedNy = model.params.calmWaterSpeedCompare; // eval_calmWaterSpeed(speedSettingNu,
		// modelDelay.arc[arcDelay].fromLevel, modelDelay.arc[arcDelay].toLevel);
	//double timeArc = modelDelay.arc[arcDelay].distance / calmWaterSpeedNy * factor;
	...
	double speedNow = calmWaterSpeedNy / factorDelay + speedDiffCurrent;
	// double timeArcCheck = modelDelay.arc[arcDelay].distance / calmWaterSpeedNy * factor;
	double timeArcCheck = modelDelay_prefPath.arc[arcDelay].distance / speedNow;
...
int copyToArcFromDelay(int posDelay, int arcNr, double timeExact, int iter) {
	int arcNu = model.nArcs, arcDelay, thisLevel, pos1, speedSetting, cNr;
	...
	if (iter != 1) {
		...
		//model.arc[arcNu].speedSetting = modelDelay.arc[arcDelay].speedSetting + model.delay.changedSpeed[posDelay];
		speedSetting = 0;
		newTime = calcNewTime_changeSpeedSetting_delay_prefPath(arcDelay, speedSetting, (int)(round(timeExact / model.params.tIndexGerH)));
	}
...
//	if (model.delay.changedSpeed[posDelay] != 0) {
		...
		if (iter != 1) {
			...
			model.arc[arcNu].fuelQualityKvot = modelDelay.arc[arcDelay].fuelQualityKvot;
		}
		else {
			...
			model.arc[arcNu].fuelQualityKvot = modelDelay_prefPath.arc[arcDelay].fuelQualityKvot;
		}
		...
		totCost = model.params.weightTime * model.params.priceTime * newTime +
...
int evalCompareSolution() {

	int i, arcNr, posReport = 0, pointNr1, pointNr2, i3, nodNr1;
	int nodNu, i1, keepGoing = 1, arcNrBas;
	double timeExact, timeOld, timeNu, time = 0, emission = 0, emissionWaiting;
	double fuel_aux = 0, fuel_auxEca = 0, fuel_eca = 0, fuel_noEca = 0, distance = 0;
	double fuelCostDollar, dollarCost, channelCost = -100000;
	...
	for (i = 0; i < model.network.nPhysicalLevels - 1; i++) {
		...
		if (i == 23)
			i = i;
		if (posDelay == 0) {
			for (i3 = 0; i3 < model.network.physicalLev[i].nTimeIntervals[pointNr1]; i3++) {
				nodNr1 = model.network.physicalLev[i].nodNr_from_pt[pointNr1][i3];
				for (i1 = 0; i1 < model.Noder[nodNr1].nUtNoder; i1++) {
					nodNu = model.Noder[nodNr1].UtNod[i1];
					if (model.Noder[nodNu].physicalLevel == i + 1 && model.Noder[nodNu].pointNr == pointNr2) {
						arcNr = model.Noder[nodNr1].outArcNr[i1];
						break;
					}
					if (model.Noder[nodNu].physicalLevel == model.network.nPhysicalLevels - 1) {
						arcNr = model.Noder[nodNr1].outArcNr[i1];
						arcNrBas = arcNr;
						copyToArcFromDelay(posDelay, arcNr, timeExact, 1);
						posDelay++;
						arcNr = model.nArcs;
						break;
					}
				}
				if (i1 < model.Noder[nodNr1].nUtNoder)
					break;
			}
			if (i3 >= model.network.physicalLev[i].nTimeIntervals[pointNr1]) {
				errlog("ERROR! Could not find an arc between levels %d and %d, I skip the comparison which otherwise will be wrong!\n", i, i + 1);
				break;
			}
		}
		else {
		...
		timeOld = timeExact;
		addPositionDataToReport(NULL, &posReport, arcNr, 0, &timeExact, "", 1);
		timeNu = timeExact - timeOld;
		...
		time += timeNu; // model.arc[arcNr].time;


		errlog("compareTime i %d timeExact %lf\n", i, timeExact);
		if (keepGoing == 0)
			break;
	}
	if (i < model.network.nPhysicalLevels - 2) {
		// set values to not found...
		model.functions.valuesNow.compare_route_endTime_h = -1;
	}
	else {
		...
		if (time > 0)
			model.functions.valuesNow.compare_averSpeed = distance / time / model.params.knots_to_km;
		else
			model.functions.valuesNow.compare_averSpeed = 0;

		model.functions.valuesNow.compare_totalDistance_kts = distance / model.params.knots_to_km;
		model.functions.valuesNow.compare_totalTime_h = time;
		...
		model.functions.valuesNow.compare_route_endTime_h = time;
	}
...
int writeSolutionToJson(std::string filename, int resAlt, char* namnSol, int iter)
{
	...
	if (SKRIV_UT_NOTHING == 0) {
		...
		fprintf(filPek, "arcPos\tspeedSetting\tcalmWaterSpeed(k)\tsogAllInclusive(k)\tdistance(k)\taccumDist(km)\tdistLeft(km)\ttime(h)\tfuelBase\temission\tsafetyBase\tchannelCost\tweightCost\tfromLevel\tfromPointNr\tfromTimeInterval\t"
			"toLevel\ttoPointNr\ttoTimeInterval\tlat1\tlon1\tlat2\tlon2\tWindFactor\tWaveFactor\tCurrentFactor\tDelayFactor\t"
			"WindSpeedDiff(k)\tWaveSpeedDiff(k)\tCurrentSpeedDiff(k)\tDelaySpeedDiff(k)\tforecastType\n");
	}
	...
	model.functions.valuesNow.sumWaveHight = 0;
	model.functions.valuesNow.maxWindSpeed = 0;
	model.functions.valuesNow.maxCurrent = 0;
	model.functions.valuesNow.maxWaveHight = 0;
	...
	for (iPos = 0; iPos < model.nBVArcs - 1; iPos++)
	{
		...
		if (SKRIV_UT_NOTHING == 0) {
			...
			fprintf(filPek, "%d\t%d\t%.3lf\t%.3lf\t%.3lf\t%.3lf\t%.3lf\t%.3lf\t%.3lf\t%.3lf\t%.3lf\t%.3lf"
				...
				model.arc[arcNr].distance / model.params.knots_to_km / model.arc[arcNr].time, 
	...
	fprintf(filpekG, "\"totalSteamingTime_h\": %.1lf, \"totalAdditionalTime_h\": %.1lf,\n", time - model.functions.valuesNow.totCorridorWaitingTime,
		model.functions.valuesNow.totCorridorWaitingTime); // time);
	if (model.functions.valuesNow.totTime_movingNoCorridors > 0.001)
		fprintf(filpekG, "\"consumptionTransit_average\": %.1lf, \"speedTransit_average\": %.1lf\n",
	...
	if (iter == 1) {
		...
		if (model.functions.valuesNow.compare_totalTime_h > 0) {
			...
			tmBas.tm_min += (-time + model.functions.valuesNow.compare_totalTime_h) * 60;
			test = mktime(&tmBas);
			if (test == -1) {
				printf("failed mktime on row %d time %d %d %d: %d %d %d\n", __LINE__,
					tmBas.tm_year,
	...
	if (SKRIV_UT_NOTHING == 0) {
		...
		if (time < 0.001)
			time = 0.001;
		fprintf(filPek, "averageWindSpeed (m/s)\t%.3lf\n", model.functions.valuesNow.sumWindSpeed / time / 3.6);
		fprintf(filPek, "averageRelCurrent (knots)\t%.3lf\n", model.functions.valuesNow.sumRelCurrent / time / 1.852);
		fprintf(filPek, "averageCurrent (knots)\t%.3lf\n", model.functions.valuesNow.sumCurrent / time / 1.852);
		fprintf(filPek, "averageWaveHight (m)\t%.3lf\n", model.functions.valuesNow.sumWaveHight / time);
		...
		fprintf(filPek, "maxWaveHight (m)\t%.3lf\n", model.functions.valuesNow.maxWaveHight);
...
{ // not used
	...
	model.network.channel = (strChannel*)malloc2(nAllocChannels * sizeof(strChannel));
...
int loadStorms_historical() {
	...
	for(i = 0; i < model.params.nHindCastMonths; i++){
		...
		if (manadNu < 10)
			sprintf(namn, "%s/weather/Hindcast/storms/storms_0%d_%d.json", model.params.indataPath.c_str(), manadNu, year);
		else
...
void setupUsableSpeedSettings() {
	...
	if (model.params.commercialAllowedVariation < 0) {
		// not commercial
		nShip_speedSettings = model.params.nSpeedSettingDivideIter1 + 1;
...
int loadVariables(int alt)
{
	...
	model.weather = (strWeather*)malloc2((int)dataVar.size() * sizeof(strWeather));
	//printf("\n####\nalloc %d weatherData\n", (int)dataVar.size());
	i0 = 0;
...
int identifyStartEndAllowed(double distInt, double* startDistBad, double* endDistBad) {
	...
	for (i = 1; i < model.preferredPath.nPoints; i++) {
		...
		if (dist + distNu > distInt) {
			...
			isAllowed = check_isStraightLineFeasible(model.preferredPath.point_y[i-1], model.preferredPath.point_x[i-1], 
	...
	for (i = model.preferredPath.nPoints - 1; i > 0; i--) {
		...
		if (dist + distNu > distInt) {
			...
			isAllowed = check_isStraightLineFeasible(model.preferredPath.point_y[i], model.preferredPath.point_x[i], 
				newPoint.latitude().degrees(), newPoint.longitude().degrees(), &distOK);
		}
		else
			isAllowed = check_isStraightLineFeasible(model.preferredPath.point_y[i], model.preferredPath.point_x[i],
...
int createPhysicalNetwork(int sparaKorridorEnbart, int alt)
{
	...
	if (startDistBad + endDistBad > 0.01) {
		nIntDbl = (distTot - (startDistBad + endDistBad)) / model.params.shipSpeed_average / model.params.nHours_changeCourseInterval;
		nInt = (int)ceil(nIntDbl);
		distInt = (distTot - (startDistBad + endDistBad)) / nInt * (1 + model.params.epsilon); // # add a small number so there will be no nodes orthogonal to the last one
		if (startDistBad > 0)
			nInt++;
		if (endDistBad > 0)
			nInt++;
		errlog("recalculate distInt, nIntDbl %lf nInt %d distInt %lf, startDistBad %.2lf endDistBad %.2lf\n", 
			nIntDbl, nInt, distInt, startDistBad, endDistBad);
	}
	...
	nAllocPoints = 100;
	model.network.physicalLev[0].preferredPathPoint = (spherical::Point*)malloc2(nAllocPoints * sizeof(spherical::Point));
	...
	for (i = 1; i < model.preferredPath.nPoints; i++) {
		...
		while (dist + distKvar >= distIntUse && nIntervallPoints < nInt) {
			...
			posNu = 0;
			nIntervallPoints++;
			if (nIntervallPoints >= 47)
				nIntervallPoints = nIntervallPoints;
			nAllocPoints = 100;
			model.network.physicalLev[nIntervallPoints - 1].preferredPathPoint = (spherical::Point*)malloc2(nAllocPoints * sizeof(spherical::Point));
			//distIntUse = distInt;
			if (dist > distInt)
				dist -= distIntUse;
			else {
...
int calcWeatherPosAlongpreferredPathArc_connectChannel(spherical::Point p1, int level1, spherical::Point p2, int level2)
{
	...
	if (level1 >= 0) {
		level = level1;
		posLast = -1;
		posEnd = model.network.channel[-level2 - 1].preferredPathPoint_posConnectTo + 1;
		if(posEnd < 1)
			totDist = model.network.physicalLev[level].point[model.params.preferredPathOrtoPos[level]].distanceTo(p2) / 1000;
		else
			totDist = model.network.physicalLev[level].preferredPathPoint[posEnd - 1].distanceTo(p2) / 1000;
		totDist2 = totDist;
	}
	else {
		level = level2 - 1;
		...
		totDist = p1.distanceTo(model.network.physicalLev[level].preferredPathPoint[posLast]) / 1000;
		totDist1 = totDist;
...
int evalDistanceBetweenPrefPathAndChannel(int level1, int level2)
{
	...
	if (level1 >= 0) {
		...
		if (posEnd < 1)
			...
			totDist = model.network.physicalLev[level].preferredPathPoint[posEnd - 1].distanceTo(model.network.channel[-level2 - 1].point[0]) / 1000;
	}
	else {
		level = level2 - 1;
		posLast = model.network.channel[-level1 - 1].preferredPathPoint_posConnectFrom - 1;
		posEnd = model.network.physicalLev[level].npreferredPathPoints;
		pMid = model.network.channel[-level1 - 1].point[model.network.channel[-level1 - 1].nPoints - 1];
		totDist = pMid.distanceTo(model.network.physicalLev[level].preferredPathPoint[posLast + 1]) / 1000;
	}
...
double calcDelayedArcTimeCostChannel(int t, int speedSettingNr, int channelNr, double delayFactor){
	...
	double timeArc, fuelConsumption_main, fuelConsumption_aux, fuelUsage_main, fuelUsage_aux;
	double tidStart = t * model.params.tIndexGerH, tidTot = tidStart, fixTime, calmWaterSpeed, dist;
...
int addEnBage_AB(int thisLevel, int pos1, int nextLevel, int pos2, int tPos, int i4, int* setupCheckPoints, int min_t, int max_t, double fuelQualityKvot, int runAlt)
{
	...
	if (thisLevel >= 0) {
		...
		if (pos1 == model.params.preferredPathOrtoPos[thisLevel]) {
			if (nextLevel >= 0) {
				...
				if ((model.network.channel[-nextLevel - 1].straightArcFeasible_toChannelFromPrefPath == 0 || model.params.max_changeDirection == 0 || runAlt == 1) &&
					pos1 == model.params.preferredPathOrtoPos[thisLevel])
					prefPath = 1;
			}
		...
		if (nextLevel >= 0) {
			...
			if ((model.network.channel[-thisLevel - 1].straightArcFeasible_fromChannelToPrefPath == 0 || model.params.max_changeDirection == 0 || runAlt == 1) &&
				pos2 == model.params.preferredPathOrtoPos[nextLevel])
				prefPath = 1;
...
int addEnBage_delayAB(int thisLevel, int pos1, int nextLevel, int pos2, int tPos, int i4, int min_t, int max_t, double fuelQualityKvot, double dist, 
	double delayFactor, double speedDiffCurrent)
{
	// i = thisLevel, i1 = pointPos, i+1 = nextLevel, i2 = outNodePos, i3 = tPos
	int tidInt, nArcsNu = 0, nodNr1, nodNr2, posNy, arcNr, prefPath = 0;
	int timeInt, nSpeedSettings;
...
int genArcs_withDelay(int thisLevel, int pos1, int nextLevel, int pos2, int tPos, int nSpeedSettings, double fuelQualityKvot, double* distArc, 
	double delayFactor, double speedDiffCurrent) {
	...
	for (i4 = 0; i4 < nSpeedSettings; i4++) {
		if (model.nArcs >= 3939)
			model.nArcs = model.nArcs;
		arcNr = addEnBage_delayAB(thisLevel, pos1, nextLevel, pos2, tPos, i4, 0, 1e10, fuelQualityKvot, *distArc, delayFactor, speedDiffCurrent);
		if (arcNr >= 0)
			nArcs++;
	}
	return nArcs;
...
int genArcsTo_delayedPreferredPath(int thisLevel, int pos1, int nextLevel, int pos2, int tPos, int nSpeedSettings, double fuelQualityKvot, 
	double* delayFactor, double* distArc)
{
	...
	for (i = 0; i < model.network.nPhysicalLevels; i++) {
		...
		for (i1 = 0; i1 < model.network.physicalLev[i].nPoints; i1++) {
			...
			fprintf(filpek, "  {\"type\":\"Feature\", \"properties\":{\"fromLevel\":%d, \"fromNodPos\":%d, \"nArcs\":%d, \"time\":%.3lf},\n",
				i, i1, routeToEnd->nBVArcs, routeToEnd->time);
	...
	for (i = 0; i < model.network.nChannels; i++) {
		...
		for (i1 = 0; i1 < 2; i1++) {
			...
			if (forsta != 1)
				...
				forsta = 0;
			fprintf(filpek, "  {\"type\":\"Feature\", \"properties\":{\"fromLevel\":%d, \"fromNodPos\":%d, \"nArcs\":%d, \"time\":%.3lf},\n",
				-i - 1, i1, routeToEnd->nBVArcs, routeToEnd->time);
			fprintf(filpek, "    \"geometry\":{\"type\": \"MultiLineString\", \"coordinates\":[[");
			addArcDelayToGeojson(filpek, routeToEnd);
			fprintf(filpek, "]]}}\n");
		}
...
int addBagar_AB_speedSTid(int thisLevel, int pos1, int nextLevel, int pos2, int* setupCheckPoints, int min_t, int max_t, double fuelQualityKvot, int runAlt) {
	...
	if (thisLevel >= 0) {
		for (i3 = 0; i3 < model.network.physicalLev[thisLevel].nTimeIntervals[pos1]; i3++) {
			if (model.network.physicalLev[thisLevel].timeInterval[pos1][i3] * model.params.tIndexGerH < model.network.tidp_startHistoricDataOnly) {
				for (i4 = 0; i4 < nSpeedSettings; i4++)
					arcNr = addEnBage_AB(thisLevel, pos1, nextLevel, pos2, i3, i4, setupCheckPoints, min_t, max_t, fuelQualityKvot, runAlt);
			}
			else {
				if (delayVersion < 3)
		...
		*setupCheckPoints = 1;
		...
		for (i3 = 0; i3 < model.network.channel[-nextLevel - 1].nTimeIntervals[0]; i3++) {
			if (model.network.channel[-nextLevel - 1].timeInterval[0][i3] * model.params.tIndexGerH < model.network.tidp_startHistoricDataOnly) {
				for (i4 = 0; i4 < nSpeedSettings; i4++)
					arcNr = addEnBage_AB(nextLevel, 0, nextLevel, 1, i3, i4, setupCheckPoints, 0, 99999, fuelQualityKvot, runAlt);
			}
			else {
				if (delayVersion < 3) {
					if (model.network.channel[-nextLevel - 1].timeThroughChannel <= -0.5) {
					...
					for (i4 = 0; i4 < nSpeedSettings; i4++) {
						nArcsNu += addEnBage_delayAB(nextLevel, 0, nextLevel, 1, i3, i4, 0, 1e10, fuelQualityKvot, distArc, delayFactor, speedDiffCurrent);
					}
				}
				else {
					if (runAlt != 1)
...
int addBastSpeed_arcDelayed(int thisLevel, int pos1, int nextLevel, int pos2, int tidInt, int nSpeedSettings, double fuelQualityKvot) {
	...
	double fuelWaitCorridor_main = 0, fuelWaitCorridor_aux = 0, totCostCompare;

	if (modelDelay.nArcs == 7974)
		modelDelay.nArcs = modelDelay.nArcs;
	...
	if (nextLevel >= 0) {
		...
		if (thisLevel >= 0) {
			if ((model.network.channel[-nextLevel - 1].straightArcFeasible_fromChannelToPrefPath == 0 || model.params.max_changeDirection == 0) &&
				pos1 == model.params.preferredPathOrtoPos[thisLevel])
				dist = evalDistanceBetweenPrefPathAndChannel(thisLevel, nextLevel);
			else {
				x1 = model.network.physicalLev[thisLevel].point_x[pos1];
	...
	for (i4 = 0; i4 < model.functions.nShip_speedSettingsBase; i4++) {
		...
		fuelConsumption_main += fuelWaitCorridor_main;
		fuelConsumption_aux += fuelWaitCorridor_aux;
...
int addBastSpeed_arcDelayed_prefPath(int thisLevel, int pos1, int nextLevel, int pos2, int tidInt, double fuelQualityKvot) {
	// loop over all speedsettings, find lowest cost with delay factor
	int i4, minPos, posTmp;
	double minCost = 1e20, totCost, channelCost, calmWaterSpeed, timeArc;
	double fuelConsumption_main, fuelConsumption_aux, fuelUsage_main, fuelUsage_aux;
	...
	// factorDelay = eval_factorDelayedAlongPath(thisLevel, tidInt);
	factorDelay = eval_factorDelayedAlongArc_currSpeedDiff(thisLevel, pos1, nextLevel, pos2, tidInt, &speedDiffCurrent,
		model.params.calmWaterSpeedCompare);
	if (nextLevel >= 0) {
		if (thisLevel >= 0) {
			if (model.params.preferredPathOrtoPos[thisLevel] == pos1 && model.params.preferredPathOrtoPos[nextLevel] == pos2)
				dist = model.network.physicalLev[thisLevel + 1].distanceFromStartPosMid - model.network.physicalLev[thisLevel].distanceFromStartPosMid;
	...
	totCost = channelCost;
	if (fixTime < -0.5) {
		calmWaterSpeed = model.params.calmWaterSpeedCompare; // eval_calmWaterSpeed(i4, -1, -100);
		speedNow = calmWaterSpeed / factorDelay + speedDiffCurrent;
		if (speedNow < 0.1)
			speedNow = 0.1;
		timeArc = dist / speedNow + waitingTime; // in hours
...
int addBage_AB_delayFysiskt(int thisLevel, int pos1, int nextLevel, int pos2, int tidInt, double fuelQualityKvot) {
	int nSpeedSettings, prefPath, i4, i3, arcNr;
	int nArcsNu = 0;
	double calmWaterSpeed = -1.0, distArc = -1, delayFactor = 0;
	double fuelFactorMain, nAddedTotArcs = 0, speedDiffCurrent = 0;
	...
	arcNr = addBastSpeed_arcDelayed(thisLevel, pos1, nextLevel, pos2, tidInt, nSpeedSettings, fuelQualityKvot);
	// loop over all speedsettings, find lowest cost with delay factor
...
void identify_hindCastMonths(tm tmEnd) {
...
void loadWeatherFiles_grib() {
	...
	long long nSecondsUTC_next, endTime_secondsUTC = model.params.UTC_secondsStart + endTime * 3600;
	identify_hindCastMonths(tmBas);
		...
		for (int i1 = 0; i1 < model.weather[ii].nFiles; i1++) {
			...
			for (int i10 = 0; i10 < model.params.nHindCastMonths; i10++) {
				if (model.params.hindCast_month[i10] < 10)
					sprintf(namn, "%s/weather/Hindcast/0%d/%s_0%d_%d.grb", model.params.indataPath.c_str(), model.params.hindCast_month[i10],
						model.weather[ii].filePos[i1].fileName, model.params.hindCast_month[i10],
						model.params.hindCast_year[i10]);
				else
					sprintf(namn, "%s/weather/Hindcast/%d/%s_%d_%d.grb", model.params.indataPath.c_str(), model.params.hindCast_month[i10],
						model.weather[ii].filePos[i1].fileName, model.params.hindCast_month[i10],
						model.params.hindCast_year[i10]);
				printf("%s\n", namn);
					...
					if (size_col < -0.1) {
						...
						for (int i2 = 0; i2 < nBands; i2++) {
							...
							if (i2 + 1 < nBands) {
								...
								if (nSecondsUTC_next < model.params.UTC_secondsStart)
									continue; // too early
							}
...
double calcEndTime_withLowestCostSpeedAlongArc(int level1, int pointNr1, int level2, int pointNr2, double timeStart) {
	...
	double x1, y1, x2, y2, speedDiffCurrent = 0;
...
int gen_midTimeArrive_old() {
	...
	for (cNr = 0; cNr < model.network.nChannels; cNr++) {
		...
		if (model.network.physicalLev[pos].requirePrefPathFeasible == 1) {
			if (pos >= 0) {
				arcOK = check_isPhysicalArcOK(pos, -cNr - 1, model.params.preferredPathOrtoPos[pos], 0, model.params.physicalMap_noDataValue); // a preferred path but through a channel
				model.network.channel[cNr].straightArcFeasible_toChannelFromPrefPath = arcOK;
			}
			pos = model.network.channel[cNr].bastEndLevel;
			if (pos >= 0) {
				arcOK = check_isPhysicalArcOK(-cNr - 1, pos, 1, model.params.preferredPathOrtoPos[pos], model.params.physicalMap_noDataValue); // a preferred path but through a channel
	...
	for (i = 0; i < model.network.nPhysicalLevels - 1; i++) {
		...
		for (i1 = 0; i1 < model.network.physicalLev[i].nPoints; i1++) {
			...
			for (i2b = 0; i2b < model.network.physicalLev[i].nOutNodes[i1]; i2b++) {
				...
				if (runAlt == 1)
					i = i;
				addBagar_AB_speedSTid(i, i1, nextLevel, i2, &setupCheckPoints, min_t, max_t, fuelQualityKvot, runAlt);
			}
		}
		...
		for (i1 = 0; i1 < model.network.nChannels; i1++) { 
			...
			for (i2b = 0; i2b < model.network.channel[cNr].nOutNodes; i2b++) {
				...
				if (runAlt != 1)
					...
					get_minMax_timeFromLevel(-1, &min_t, &max_t);
				addBagar_AB_speedSTid(-cNr - 1, 1, nextLevel, model.network.channel[cNr].outNode[i2b], &setupCheckPoints, min_t, max_t, fuelQualityKvot, runAlt);
			}
		}
	...
	for (int iter = 0; iter < 3; iter++) {
		...
		for (int ii = 0; ii < 1 + nExtraOpt; ii++) {
			...
			if (Reached == true) {
				//printf("har1\n");
				//printf("har12\n");
				//model.filpek = fopen("checkOptArcCosts.txt", "a+");
...
int solve_SP_delay() {
	...
	modelDelay.BVArc = (int*)malloc2((model.network.nPhysicalLevels + 2) * sizeof(int));
	modelDelay.BVtempNodOrder = (int*)malloc2((model.network.nPhysicalLevels + 2) * sizeof(int));
	...
	for (i = 0; i < model.network.nPhysicalLevels - 1; i++) {
		...
		for (i1 = 0; i1 < model.network.physicalLev[i].nPoints; i1++) {
			for (i2b = 0; i2b < model.network.physicalLev[i].nOutNodes[i1]; i2b++) {
				...
				setupCheckPoints = 1;
				fuelQualityKvot = get_fuelQualityKvot(i, i1, nextLevel, i2);
				addBage_AB_delayFysiskt(i, i1, nextLevel, i2, tidInt, fuelQualityKvot);
			}
		}
		...
		for (i1 = 0; i1 < model.network.nChannels; i1++) {
			...
			for (i2b = 0; i2b < model.network.channel[cNr].nOutNodes; i2b++) {
				...
				if (nextLevel != i + 1)
					continue;
				fuelQualityKvot = get_fuelQualityKvot(-cNr - 1, 1, nextLevel, model.network.channel[cNr].outNode[i2b]);
				addBage_AB_delayFysiskt(-cNr - 1, 1, nextLevel, model.network.channel[cNr].outNode[i2b], tidInt, fuelQualityKvot);
			}
		}
	...
	for (i1 = 0; i1 < model.network.nChannels; i1++) {
		...
		addBage_AB_delayFysiskt(-cNr - 1, 0, -cNr - 1, 1, tidInt, fuelQualityKvot);
	}
	...
	//	fprintf(filpek, "arc %d from %d %d to %d %d cost %.3lf\n", i,
...
int solve_SP_delayPrefPath() {
	...
	modelDelay_prefPath.BVArc = (int*)malloc2((model.network.nPhysicalLevels + 2) * sizeof(int));
	modelDelay_prefPath.BVtempNodOrder = (int*)malloc2((model.network.nPhysicalLevels + 2) * sizeof(int));
	...
	//for (i = 0; i < 1; i++) {
		...
		for (i1 = 0; i1 < model.network.physicalLev[i].nPoints; i1++) {
			...
			for (i2b = 0; i2b < model.network.physicalLev[i].nOutNodes[i1]; i2b++) {
				...
				setupCheckPoints = 1;
				fuelQualityKvot = get_fuelQualityKvot(i, i1, nextLevel, i2);
				addBastSpeed_arcDelayed_prefPath(i, i1, nextLevel, i2, tidInt, fuelQualityKvot);
			}
		}
		...
		for (i1 = 0; i1 < model.network.nChannels; i1++) {
			...
			for (i2b = 0; i2b < model.network.channel[cNr].nOutNodes; i2b++) {
				...
				fuelQualityKvot = get_fuelQualityKvot(-cNr - 1, 1, nextLevel, model.network.channel[cNr].outNode[i2b]);
				addBastSpeed_arcDelayed_prefPath(-cNr - 1, 1, nextLevel, model.network.channel[cNr].outNode[i2b], tidInt, fuelQualityKvot);
			}
		}
	...
	for (i1 = 0; i1 < model.network.nChannels; i1++) {
		...
		fuelQualityKvot = get_fuelQualityKvot(-i1 - 1, 0, -i1 - 1, 1);
		tidInt = (int)(round(model.network.channel[i1].midTimeArrive / model.params.tIndexGerH));
		addBastSpeed_arcDelayed_prefPath(-cNr - 1, 0, -cNr - 1, 1, tidInt, fuelQualityKvot);
	}
